...
def test_check_leasing_dues_runs_without_error(db):
    """_check_leasing_dues — دلوقتي logger.info بس (كانت بتبعت واتساب —
    اتشالت 2026-09-06، الإيجار مش جزء من نطاق "الملكية الجزئية" اللي
    Mohamed طلب إبقاء الواتساب شغال فيه بس)."""
...
def test_check_leasing_dues_no_error_if_paid(db):
    """_check_leasing_dues — يشتغل بدون exception حتى لو كل الدفعات مدفوعة"""
    from app.tasks.finance_tasks import _check_leasing_dues
    from app.modules.leasing import crud as lease_crud
    from app.modules.leasing.schemas import LeaseContractCreate
    from app.modules.leasing.models import LeasePayment
