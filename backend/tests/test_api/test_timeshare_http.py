...
class TestTimeshareContractFlow:
    def test_create_contract_generates_installment_schedule(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        ...
        get_resp = client.get(f"/api/v1/timeshare/contracts/{contract['id']}", headers=timeshare_admin_headers)
        assert get_resp.status_code == 200
        assert len(get_resp.json()["installments_list"]) == 4

    def test_pay_installment_updates_status(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        branch = make_branch_committed(db)
    ...
    def test_cancel_contract_via_http(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        branch = make_branch_committed(db)
        contract = client.post(
            "/api/v1/timeshare/contracts", json=contract_payload(branch.id), headers=timeshare_admin_headers,
        ).json()
...
class TestTimeshareUnitTransfer:
    ...
    def test_transfer_succeeds_between_same_room_type_units(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        branch = make_branch_committed(db)
        old_unit, new_unit, _, _ = self._make_units(db, branch)
        contract = self._make_fixed_contract(client, branch.id, old_unit.id, timeshare_admin_headers)
    ...
    def test_transfer_rejects_different_room_type(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        branch = make_branch_committed(db)
    ...
    def test_transfer_rejects_unit_under_maintenance(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        branch = make_branch_committed(db)
    ...
    def test_transfer_rejects_floating_contract(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
    ...
    def test_transfer_rejects_when_upcoming_visit_exists(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        branch = make_branch_committed(db)
        old_unit, new_unit, _, _ = self._make_units(db, branch)
        contract = self._make_fixed_contract(client, branch.id, old_unit.id, timeshare_admin_headers)
    ...
    def test_transfer_requires_manager(self, client: TestClient, db, fake_redis, timeshare_admin_headers, cashier_headers):
        branch = make_branch_committed(db)
        old_unit, new_unit, _, _ = self._make_units(db, branch)
        contract = self._make_fixed_contract(client, branch.id, old_unit.id, timeshare_admin_headers)
...
class TestTimesharePermissions:
    ...
    def test_cancel_contract_requires_manager(self, client: TestClient, db, fake_redis, timeshare_admin_headers, cashier_headers):
        branch = make_branch_committed(db)
        contract = client.post(
            "/api/v1/timeshare/contracts", json=contract_payload(branch.id), headers=timeshare_admin_headers,
        ).json()
    ...
    def test_pay_installment_requires_cashier_not_just_active_user(
        self, client: TestClient, db, fake_redis, timeshare_admin_headers, waiter_headers,
    ):
...
class TestTimeshareVisitAndUnitsHttp:
    ...
    def test_double_booking_same_unit_rejected_via_http(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        ...
        client.patch(
            ...
            json={"unit_id": unit.id}, headers=timeshare_admin_headers,
        )
...
class TestTimeshareValidation:
    def test_create_contract_rejects_invalid_room_type(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        ...
        resp = client.post("/api/v1/timeshare/contracts", json=payload, headers=timeshare_admin_headers)
        assert resp.status_code == 422

    def test_create_contract_rejects_down_payment_exceeding_total(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
    ...
    def test_pay_installment_rejects_invalid_method(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        branch = make_branch_committed(db)
        contract = client.post(
            "/api/v1/timeshare/contracts", json=contract_payload(branch.id), headers=timeshare_admin_headers,
        ).json()
...
class TestTimeshareReportingHttp:
    ...
    def test_cs_summary_reflects_real_aggregates(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
    ...
    def test_sales_dashboard_pipeline_counts_by_status(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        ...
        branch = make_branch_committed(db)
        c1 = client.post("/api/v1/timeshare/contracts", json=contract_payload(branch.id), headers=timeshare_admin_headers).json()
        client.post("/api/v1/timeshare/contracts", json=contract_payload(branch.id), headers=timeshare_admin_headers)
        to_cancel = client.post("/api/v1/timeshare/contracts", json=contract_payload(branch.id), headers=timeshare_admin_headers).json()
        cancel = client.post(
            f"/api/v1/timeshare/contracts/{to_cancel['id']}/cancel",
            json={"cancel_amount": "5000.00"}, headers=timeshare_admin_headers,
        )
        assert cancel.status_code == 200, cancel.text

        resp = client.get(f"/api/v1/timeshare/sales-dashboard?branch_id={branch.id}", headers=timeshare_admin_headers)
        assert resp.status_code == 200, resp.text
        data = resp.json()
    ...
    def test_sales_dashboard_export_returns_valid_excel_for_manager(
        ...
        resp = client.get(
            f"/api/v1/timeshare/sales-dashboard/export?branch_id={branch.id}", headers=timeshare_admin_headers,
        )
        assert resp.status_code == 200, resp.text
    ...
    def test_calendar_shows_booked_week_with_contract(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        """عقد بأسبوع ثابت 28 → total_booked_weeks=1 والعقد يظهر تحت أسبوع 28."""
        branch = make_branch_committed(db)
        payload = contract_payload(branch.id)
        payload["week_number"] = 28
        contract = client.post("/api/v1/timeshare/contracts", json=payload, headers=timeshare_admin_headers).json()

        resp = client.get(f"/api/v1/timeshare/calendar?branch_id={branch.id}&year=2026", headers=timeshare_admin_headers)
        assert resp.status_code == 200, resp.text
        data = resp.json()
    ...
    def test_upcoming_visits_lists_active_contract_in_window(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        ...
        resp = client.get(f"/api/v1/timeshare/upcoming-visits?branch_id={branch.id}&days=365", headers=timeshare_admin_headers)
        assert resp.status_code == 200, resp.text
        visits = resp.json()
    ...
    def test_upcoming_visits_excludes_cancelled_contract(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        ...
        client.post(
            f"/api/v1/timeshare/contracts/{contract['id']}/cancel",
            json={"cancel_amount": "0"}, headers=timeshare_admin_headers,
        )
        resp = client.get(f"/api/v1/timeshare/upcoming-visits?branch_id={branch.id}&days=365", headers=timeshare_admin_headers)
        assert resp.status_code == 200
        assert not any(v["contract_number"] == contract["contract_number"] for v in resp.json())

    def test_stats_partner_share_room_and_collection(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        ...
        resp = client.get(f"/api/v1/timeshare/stats?branch_id={branch.id}", headers=timeshare_admin_headers)
        assert resp.status_code == 200, resp.text
        data = resp.json()
...
class TestTimeshareRouterMiscHttp:
    ...
    def test_list_contracts_paginated(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        branch = make_branch_committed(db)
        client.post("/api/v1/timeshare/contracts", json=contract_payload(branch.id), headers=timeshare_admin_headers)
        resp = client.get(f"/api/v1/timeshare/contracts?branch_id={branch.id}&page=1&size=20", headers=timeshare_admin_headers)
        assert resp.status_code == 200, resp.text
        body = resp.json()
        assert body["total"] == 1
        assert len(body["items"]) == 1

    def test_get_contract_404(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        make_branch_committed(db)
        resp = client.get("/api/v1/timeshare/contracts/999999", headers=timeshare_admin_headers)
        assert resp.status_code == 404

    def test_update_nonexistent_contract_returns_404(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        ...
        make_branch_committed(db)
        resp = client.patch(
            "/api/v1/timeshare/contracts/999999", json={"notes": "x"}, headers=timeshare_admin_headers,
        )
        assert resp.status_code == 404

    def test_pay_already_paid_installment_returns_400(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        branch = make_branch_committed(db)
        contract = client.post(
            "/api/v1/timeshare/contracts", json=contract_payload(branch.id), headers=timeshare_admin_headers,
        ).json()
        ...
        first = client.post(
            ...
            json={"paid_amount": inst["amount"], "payment_method": "cash"}, headers=timeshare_admin_headers,
        )
    ...
    def test_waitlist_add_then_list(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        branch = make_branch_committed(db)
        contract = client.post(
            "/api/v1/timeshare/contracts", json=contract_payload(branch.id), headers=timeshare_admin_headers,
        ).json()
        ...
        lst = client.get(f"/api/v1/timeshare/waitlist?branch_id={branch.id}", headers=timeshare_admin_headers)
        assert lst.status_code == 200
        assert len(lst.json()) == 1

    def test_waitlist_invalid_dates_returns_400(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
    ...
    def test_download_contract_pdf(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        ...
        resp = client.get(f"/api/v1/timeshare/contracts/{contract['id']}/pdf", headers=timeshare_admin_headers)
        assert resp.status_code == 200, resp.text
        assert resp.headers["content-type"] == "application/pdf"
        assert resp.content[:4] == b"%PDF"

    def test_download_pdf_nonexistent_returns_404(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        make_branch_committed(db)
        resp = client.get("/api/v1/timeshare/contracts/999999/pdf", headers=timeshare_admin_headers)
        assert resp.status_code == 404

    def test_list_installments_http(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        branch = make_branch_committed(db)
        client.post("/api/v1/timeshare/contracts", json=contract_payload(branch.id), headers=timeshare_admin_headers)
        resp = client.get(f"/api/v1/timeshare/installments?branch_id={branch.id}", headers=timeshare_admin_headers)
        assert resp.status_code == 200, resp.text
        body = resp.json()
        assert body["total"] == 4
    ...
    def test_list_installments_includes_customer_for_frontend_table(
        ...
        client.post("/api/v1/timeshare/contracts", json=payload, headers=timeshare_admin_headers)
        resp = client.get(f"/api/v1/timeshare/installments?branch_id={branch.id}", headers=timeshare_admin_headers)
        assert resp.status_code == 200, resp.text
    ...
    def test_list_visits_http(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        ...
        resp = client.get(f"/api/v1/timeshare/visits?branch_id={branch.id}", headers=timeshare_admin_headers)
        assert resp.status_code == 200
        assert len(resp.json()) == 1

    def test_update_nonexistent_visit_returns_404(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        make_branch_committed(db)
        resp = client.patch(
            "/api/v1/timeshare/visits/999999", json={"status": "completed"}, headers=timeshare_admin_headers,
        )
        assert resp.status_code == 404

    def test_import_contracts_excel(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        import io
        import openpyxl
        branch = make_branch_committed(db)
    ...
    def test_import_excel_missing_columns_returns_400(self, client: TestClient, db, fake_redis, timeshare_admin_headers):
        import io
        import openpyxl
        branch = make_branch_committed(db)
        wb = openpyxl.Workbook()
