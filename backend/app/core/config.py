...
class Settings(CoreSettings):
    ...
    # صغير (تفكة/تقريب) طبيعي في أي تعامل نقدي يومي — تحذير بس. فرق كبير
    # *نسبةً لحجم مبيعات الوردية نفسها* (مش رقم ثابت — وردية بمبيعات 50,000 ج
    # مختلفة تمامًا عن وردية بمبيعات 200 ج) بيدل على نقص كاش حقيقي أو غلط
    # جسيم في العدّ، ولازم يترفض القفل خالص لحد ما مدير يراجع الموقف.
    CASH_VARIANCE_WARNING_ABS: float = 50.0     # ج — فوق كده = تحذير (بيقفل برضو)
