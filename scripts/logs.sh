...
case "$MODE" in
  ...
  all|*)
    print_header
    # ملاحظة: frontend-public أُزيل (2026-07-26) — الموقع العام الجديد
    # (elkheima-marketing-website) لديه logs مستقلة على terminal منفصل.
    for f in api celery celery_beat frontend-el-kheima; do
      touch "$LOG_DIR/$f.log"
    done
    if command -v multitail >/dev/null 2>&1; then
      ...
      tail -f \
        ...
        "$LOG_DIR/frontend-el-kheima.log" \
      | awk '
