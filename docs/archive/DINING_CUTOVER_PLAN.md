...
> **الحالة: ✅ منفَّذة بالكامل (2026-07-13)** — D-01 → D-08 كلهم اتعملوا، بإذن صريح من Mohamed.
> `restaurant`/`cafe` اتحذفوا نهائيًا من الكود (الجداول لسه موجودة فعليًا في Postgres كأرشيف/
> safety-net، مش متحذوفتش). راجع `CLAUDE.md` §18 — قسم "cutover الكامل D-05 → D-08" (2026-07-13)
> — للملخص الكامل: 6 دفعات (سد فجوات تكافؤ، تحقق بيانات، تحويل مالي، تبديل الافتراضي، تحقق حي،
> الحذف الفعلي)، ونقاط الرجوع (git tag `pre-dining-cutover-2026-07-12`، نسخة `pg_dump`، نسخة مرجعية
> في `/home/wego/projects/restaurant-os/reference-from-resort-os-2026-07-12/`).
>
> باقي هذا الملف اتسايب زي ما هو **كسجل تاريخي** — التحليل والمقترح الأصلي اللي بُني عليه قرار
> D-05 (تحويل analytics/finance، تحت)، مش وصف للوضع الحالي. المُنفَّذ وقتها (Batch A، D-01 → D-04،
> 2026-07-12): موديول `app/modules/dining/` — models/schemas/crud/services/router — بُني جنب
> `restaurant`/`cafe` الأصليين، بدون ما يلمسهم. تفاصيل التحقق الكاملة (تست، migration، إلخ) في
> نهاية هذا الملف.
