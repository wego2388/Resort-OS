...
مراجعة مستقلة قبل الاعتماد. Gate 5 Batch 8 (خريطة/POS الشاطئ) منفَّذة
ومُتحقَّق منها ذاتيًا (2026-07-20) — الباقي (~32 شاشة) دفعات لاحقة.
...
| Gate 5: Staff UX batches | **Batch 8 (خريطة/POS الشاطئ) منفَّذة ومُتحقَّق منها ذاتيًا (2026-07-20)** — الباقي (~32 شاشة) لسه لاحق | Claude | دفعات صغيرة ثنائية اللغة قابلة للاختبار |
...
**Batch 7 — إدارة الشاطئ (2026-07-20)، منفَّذة ومُتحقَّق منها ذاتيًا، فرع
`gate-5-staff-ux-batch-7-beach-admin-i18n`:** `BeachAdminView.vue`. نفس فئة
مخالفات Gate 3 (`dir="rtl"` ثابت) زائد تعارض اسم `t` مع دالة الترجمة في
`switchTab` اتكشفت واتصلحت.

**Batch 8 — خريطة/POS الشاطئ (2026-07-20)، منفَّذة ومُتحقَّق منها ذاتيًا، فرع
`gate-5-staff-ux-batch-8-beach-map-pos-i18n`:** `BeachMapView.vue` و
`BeachPOSView.vue`. نفس فئة مخالفات Gate 3 (`dir="rtl"` ثابت، `ar-EG`) زائد
تعارض اسم `t` في دالتين (`typeLabel`/`typeIcon`) وphysical CSS
(`text-right`/`mr-1`) اتكشفت واتصلحت.
