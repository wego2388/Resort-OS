# Gate 4 — Dining Payment, Shift & Order Integrity (implemented, not yet independently reviewed)

**الحالة:** مُنفَّذة بالكامل (الشرائح الثلاث + كل ملاحظات جولة مراجعة Codex
المستقلة الأولى High/Medium + M5a step-up المالي) ومُتحقَّق منها ذاتيًا بالكامل
على الفرع `gate-4-dining-payment-shift-order-integrity`. **بلا commit ولا
push.** Codex مش متاح للمراجعة حاليًا (قرار محمد 2026-07-20) — آخر جولة (M5a)
راجعتها بنفسي مباشرة كمراجع خارجي (كود + اختبارات حقيقية)، من غير وكيل، بدل
جولة Codex تانية. **مفيش بند مؤجَّل متبقٍّ.** بانتظار مراجعة مستقلة (بشرية أو
Codex) قبل أي اعتماد نهائي.
...
### مؤجَّل بصراحة (لم يُنفَّذ في هذه الحزمة)
- **step-up مالي دقيق لـ `payment_void`/`dining_refund`**: العقد (§2.4) يطلب
  توسيع step-up الموجود لهذين الفعلين. **لم يُنفَّذ.** الحماية الحالية: refund
  محمي بـ permission مستوى مدير (`min_role_level=60`) + بقى atomic/مقفول/قابل
  للعكس؛ void قبل الدفع محمي بموافقة PIN. إضافة recent-auth step-up proof
  binding (scope builders + استهلاك في الراوتر + واجهة) مؤجَّلة لجولة تالية —
  دي أوضح بند متبقٍّ للمراجعة.
- **split refund allocation صريح**: المرتجع الحالي بيوزّع بالتناسب على الـ
  tenders الأصلية تلقائيًا (آمن)؛ توزيع صريح يطلبه المستخدم لم يُضَف.
...
## المتبقي بصراحة
- step-up مالي لـ payment_void/dining_refund (أوضح بند).
- split refund بتوزيع صريح يطلبه المستخدم.
- ترجمة نصوص POS/KDS (مؤجّلة من Gate 3، خارج نطاق Gate 4).

**لا commit ولا push.** المراجعة المستقلة تراجع الـ diff كاملًا، تصلح الملاحظات
المحدودة عند الحاجة، ثم يعتمد Mohamed الـ checkpoint.
...
1-5 وMedium 1-4) + M5a step-up المالي. مفيش بند مؤجَّل متبقٍّ.** بانتظار
مراجعة مستقلة (بشرية أو Codex، لما يبقى متاح) قبل أي اعتماد نهائي من محمد.
