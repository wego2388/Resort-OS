...
شقيق لدفعات 1-14، لسه محتاج دمج). بلا commit/push خارج الفرع.
...
**تحقق شامل قبل الـ commit**: `node scripts/validate-i18n.mjs` نظيف (3116
مفتاح ar/en، صفر ناقص، صفر مخالفة اتجاه فيزيائي، 12 STRICT_FILES)، `pnpm run
type-check:all` نظيف، `npx vitest run` → 60/60، `pnpm run build:all` نظيف
لـ `el-kheima` و`public`، `git diff --check` نظيف.

**Gate 5 كامل — كل 15 دفعة منفَّذة على فروع شقيقة منفصلة** (1-15، متفرّعة كل
واحدة من نفس قاعدة Gate 4، مش متسلسلة فوق بعض). **الخطوة التالية المتبقية
فعليًا**: دمج الـ 15 فرع في فرع واحد (أو تسلسل واحد) قبل أي commit/push
نهائي أو نشر للإنتاج — لسه ما اتعملتش، ومحتاجة مراجعة/قرار من محمد قبل
البدء فيها لأنها عملية دمج كبيرة عبر ~50+ ملف.
