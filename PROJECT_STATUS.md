...
> **آخر تحديث حقيقي:** 2026-07-04 — كل رقم في الملف ده اتأكد منه فعليًا (تشغيل تست، تجربة live)،
...
| **الاختبارات** | **~910 اختبار، كلهم شغالين** ✅ |
...
| **finance** (الحسابات) | 772+ | 45+ | ✅ الأقوى — إهلاك أصول + تسوية بنكية جديدين (باك إند بس، الفرونت إند لسه) |
| **inventory** (المخزون) | 484 | 25 | ✅ قوي |
| **hr** (الموارد البشرية) | 455 | 39 | ✅ قوي، فيه self-service للموظف دلوقتي |
| **restaurant** (المطعم) | 476+ | 28+ | ✅ الأنضج — order→kitchen→payment كامل، إيصال حراري حقيقي، مرتجع بعد الدفع |
| **beach** (الشاطئ) | 475+ | 18 | ✅ قوي — إلغاء بيعكس الأثر المالي فعليًا الآن (كان باج حقيقي)، إيصال حراري |
| **cafe** (الكافيه) | 310+ | 18+ | ✅ منيو حقيقي (60 صنف/9 أقسام)، POS كامل شغال، إيصال حراري |
| **analytics** (التحليلات) | 264 | 16 | 🟡 متوسط |
| **timeshare** (التايم شير) | 477 | 19 | ✅ قوي |
| **pms** (الفنادق/الغرف) | 273 | 16 | 🟡 متوسط |
| **crm** (العملاء) | 173+ | 22+ | ✅ Campaigns بقت ميزة حقيقية كاملة (كانت جدول ميت) |
| **maintenance** (الصيانة) | 183 | 14 | 🟡 باك إند قوي، **بدون أي فرونت إند خالص** — فجوة حقيقية |
...
- **موديول الصيانة (maintenance) بدون فرونت إند خالص** — الباك إند كامل (أصول، أوامر شغل، جداول
  صيانة وقائية، 14 endpoint) لكن مفيش أي شاشة/route/عنصر قائمة في `el-kheima` — الموظفين معندهمش أي
  طريقة يستخدموا الميزة دي فعليًا رغم إنها جاهزة بالكامل من ناحية الباك إند.
- **إهلاك الأصول + التسوية البنكية بدون فرونت إند** — اتبنى الباك إند بالكامل النهاردة (2026-07-04)
  واتعمله تست، بس مفيش شاشة لتشغيل دورة الإهلاك الشهرية أو إدارة الحسابات البنكية/التسوية.
- **فجوات تغطية اختبارات حقيقية** (من مراجعة خارجية مستقلة، 2026-07-04) — HR router 57% coverage
  (الأضعف في المشروع، وأحسّاس موضوع لأنه بيمس رواتب وحقوق موظفين — أهم الأجزاء الغايبة: payroll
  approve، shift swaps، rota assignments)، الشاطئ B2B endpoints (عقود شركات خارجية، حصص، فاتورة
  شهرية) أقل تغطية من باقي الموديول، مطعم/كافيه hold+refund+WebSocket KDS مباشر مش مغطيين كفاية.
...
14 موديول (كلهم دايمًا شغالين، مفيش تفعيل/تعطيل)، ~910 اختبار شغالين. اتأكد من أي حاجة بتشتغل عليها
