...
تحت `_services/`، 2026-09-07) كلهم بقوا façade رفيع بيعيد تصدير نفس
الأسماء — نقل كود حرفي بدون أي تغيير سلوك، صفر رجوع (`pytest tests/ -v`
أخضر بالكامل بعد كل دفعة). Frontend: نفس النمط بالظبط اتطبّق على آخر 4
شاشات ضخمة كانت متبقية — `FinanceView.vue` (3518→181 سطر، 18 تاب)،
`CRMView.vue` (1495→63، 7 تابات)، `HRView.vue` (1700→57، 5 تابات)،
`TimeshareView.vue` (2129→106، 10 تابات) — كل تاب بقى مكوّن مستقل بيحمّل
بياناته لوحده بدل تحميل شامل واحد. قبل تقسيم timeshare، اتحل موضوع
worktree معلّق (`resort-os-rel15-auth-readiness`) كان واقف عليه من فترة:
اتسحب منه تصليح حقيقي غير محفوظ (منحنى sparkline ناعم + أيقونات SVG
حقيقية بدل إيموجي في تطبيق المالك، رد فعل مباشر على ملاحظة Mohamed
السابقة) ودليل UAT شخصي، والـworktree اتشال بعد التأكد إن باقي فرعه
مدموج في main من الأساس.

بعدها، بطلب صريح من Mohamed ("شوف مشاكل تانية كمطور... فكر ولو لقيت
حاجة قولي")، جولة تدقيق ذاتي شاملة (مش تكرار لـSEC-01→13 المُصلَّحة قبل
كده — راجع فوق) كشفت 2 باج حقيقيين اتصلحوا فورًا مع اختبار انحدار جديد
لكل واحد:
1. **IDOR حقيقي في `GET /owner/shifts/{id}/invoices`** (نفس فئة باجات
   C-01/H-01/SEC-06/SEC-07 المُصلَّحة قبل كده) — endpoint جديد (اتضاف
   2026-09-05) كان بيقرا فواتير أي وردية في أي فرع بمجرد تخمين shift_id،
   من غير أي تحقق فرع خالص (`bypass_ownership_check=True` كانت بتشيل قيد
   finance's الخاص بيها كمان). اتصلح بإضافة تحقق ملكية فرع صريح — 404
   لو الوردية في فرع تاني.
2. **تقرير أعمار الديون كان بيقع على `date.today()` الخام** (توقيت
   السيرفر UTC) بدل `business_today(settings.TIMEZONE)` لما `as_of`
   يتسيب فاضي — نفس فئة باج §13 التوقيت المتكررة (اتصلحت 6+ مرات في
   أماكن تانية من المشروع). أثر منخفض حاليًا (الفرونت إند الوحيد اللي
   بيستخدم الـendpoint ده دايمًا بيبعت `as_of` صريح)، بس عقد API عام غلط
   لأي مستخدم تاني يتسيب الباراميتر فاضي. اتصلح.

`pytest tests/ -v` → **2918 عدّوا، 0 فشل** (بعد كل تعديلات هذه الدفعة
مجتمعة). تفاصيل كاملة (خطة التقسيم لكل ملف، القرارات المعمارية —
duplicate-fetch لبيانات مشتركة، duplicate-modal للمودالات المستخدمة من
تابين، إلخ) في محادثة الجلسة — مفيش handoff doc منفصل، الشغل ده كان
جلسة واحدة متصلة مش تسليم بين وكلاء متوازيين.

**السابق:** 2026-08-31 — **SEC-01→SEC-13: كل الـ12 finding من جولة
