...
**آخر تحديث:** 2026-08-30 — REL-23: مراجعة Codex مستقلة (طلبها محمد
بعد REL-22) لقت 1 Critical + 7 High + 4 Medium. Claude تحقق شخصيًا من
كل finding بقراءة الكود قبل أي تنفيذ. Mohamed وافق على الدفعة 1 بس
(C-01 — عزل الفروع في Finance) — اتنفّذت: ~30 endpoint كانوا من غير أي
فحص عزل فرع خالص (أوسع بكتير مما وصفه Codex نفسه، شمل كل التقارير
المالية الرئيسية)، زائد فحص تكامل بيانات جديد (account_id/cost_center_id
لازم يتبعوا نفس فرع القيد). 6 اختبارات انحدار جديدة + 16 اختبار موجود
اتصلح (سلوك صحيح جديد). `pytest tests/ -v` أخضر بالكامل. **الكود
commit فقط — لسه محتاج push/deploy، والدفعات 2-5 (11 finding متبقية)
لسه معلّقة لحد موافقة محمد.** تفاصيل كاملة:
