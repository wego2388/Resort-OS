...
## 🟥→🟩 اكتشاف حرج: دليل الحسابات كان فاضي بالكامل على الإنتاج — صفر قيد يومية من أول تشغيل (2026-07-27)

Mohamed سأل عن حالة قرار "حساب إيراد الغرف الموحّد" (المؤجَّل من 2026-07-07، راجع CLAUDE.md §18
النسخة القديمة). الفحص كشف حاجتين مختلفتين:

**الخبر الحلو**: التصميم نفسه (حساب أب `4000 الإيرادات` بمسارين واضحين — `4100` لحجوزات PMS
العادية عبر Night Audit اليومي، `4600`/`4650` لعقود التايم شير) **اتنفّذ فعليًا في الكود** بتاريخ
2026-07-25/26 (commits `e83020c`/`9c97df1`) — بالظبط مطابق لقرار Mohamed، مع `Account.parent_id`
حقيقي مش تسمية شكلية بس.

**الاكتشاف الحرج**: فحص مباشر لقاعدة بيانات الإنتاج الحقيقية (SQL خام، مش افتراض) لقى **0 حساب و0
قيد يومية على الإطلاق** من أول ما السيرفر اشتغل. السبب: `python -m app.seed` (اللي بيزرع دليل
الحسابات) ممنوع صراحةً من الشغل على production بتصميم (حماية من بيانات تجريبية وهمية) — ومفيش أي
آلية بديلة كانت بتزرع دليل الحسابات على الإنتاج الحقيقي. وبما إن `post_simple_revenue_journal`
(الدالة المشتركة لكل الـ6 موديولات: مطعم/كافيه/شاطئ/PMS/تايم شير/إيجارات) مصمّمة عمدًا تبتلع أي
خطأ ترحيل بصمت (عشان فشل القيد ميوقفش البيع الحقيقي) — **كل عملية بيع حقيقية حصلت فعليًا على
الإنتاج كانت بتنجح تشغيليًا 100% بصفر أثر محاسبي خالص**، من غير أي رسالة خطأ ظاهرة لحد ما.

**الحل (بموافقة Mohamed الصريحة نفس اليوم)**:
1. شغّلت `_seed_chart_of_accounts` (فنكشن معزولة idempotent من `app/seed.py`، بدون أي بيانات
   تجريبية أخرى) مباشرة ضد production → 28 حساب حقيقي اتزرعوا بالتسلسل الصح (4 حسابات أب + 24
   حساب فرعي).
2. اتأكد الترحيل شغال حي: نداء حقيقي لـ`post_simple_revenue_journal` رجع `JournalEntry` حقيقي
   (id=1)، بعدين `db.rollback()` عشان مايتسجّلش فعليًا (صفر أثر بعد التحقق).
3. **قرار موثّق صراحةً**: مفيش backfill رجعي للمعاملات القديمة اللي فاتت من غير قيد — نفس منطق
   قرار عدم backfill `cost_center_id` القديم (تعقيد غير مبرر لبيانات ما قبل الإصلاح). الإصلاح
   سارٍ على أي معاملة جديدة من دلوقتي بس.

**تحديث توثيقي**: الفقرة المؤجَّلة في CLAUDE.md §18 (بند 0 القديم) اتشالت بالكامل واتحطّ مكانها
دخول في قسم "✅ مكتمل" — القرار نفّذ وتأكد حي، مش مؤجَّل تاني.

---

## 🟩 مراجعة نهائية (كود ميت + جودة) + Gate 8 guest alerts متأكد منه حي + أول commit/push حقيقي (2026-07-27، دفعة تانية)

كمّلة لنفس يوم النشر فوق — بعد الاستقرار، مراجعة نهائية شاملة للكود + تفعيل ميزة كانت جاهزة بس
مقفولة، زائد أول commit/push حقيقي لكل الشغل المتراكم (من هنا ومن Codex) بعد أسابيع بدون git.

**مراجعة جودة الباك إند** (`ruff` اتنصّبت مؤقتًا للمراجعة): 13 import غير مستخدم اتصلحوا،
صفر TODO/FIXME حقيقي، صفر كود متعلَّق حقيقي. **باجين حقيقيين اتلقوا أثناء تتبّع تست كان بيفشل
بالصدفة فقط لما السويت الكامل يتشغّل**:
- `_check_timeshare_dues`/`_check_leasing_dues` (`app/tasks/finance_tasks.py`) كانوا بياخدوا
  `branch_id` كـparameter من غير أي استخدام فعلي في الاستعلام — مع أكتر من فرع نشط، كل فرع كان
  بيعيد إرسال تذكير واتساب لكل الأقساط المستحقة في *كل* الفروع، يعني العميل ياخد رسالة مكررة
  بعدد الفروع النشطة. اتصلح بـjoin على العقد + فلترة `branch_id` فعلية (وشال N+1 query كمان).
- `GET /analytics/reviews/survey-token/{booking_id}` و`.../timeshare/{visit_id}` (توليد رابط
  استبيان الرضا) كانوا بيقبلوا أي `branch_id` من غير تحقق إن الحجز/الزيارة فعلاً بتاعته — مستخدم
  مسجّل دخول بأي صلاحية كان يقدر يولّد token صحيح لحجز فرع تاني، يلوّث إحصائيات فرع مش بتاعه.
  اتصلح بنفس نمط التحقق الموجود بالفعل في endpoint تالت جنبه (`send_timeshare_survey`).
- 4 اختبارات جديدة للباجين دول، الفول سويت الكامل عدّى **صفر فشل** (كان فيه فشل واحد متقطع بسبب
  التلوّث ده تحديدًا، مش موجود تاني).

**مراجعة جودة الفرونت إند**: وكيل خلص شغل i18n/دارك مود شامل (`ShiftPanel.vue` كان صفر ترجمة
خالص، `ShiftMonitorView.vue` كان بيستخدم namespace مفاتيح غلط فظهر نص خام، باج حقيقي في
scoped CSS كان بيلغي دارك مود شاشات `/pos/*`/`/waiter/*` بالكامل). أنا كمان صلحت 20+ متغيّر/import
غير مستخدم في `el-kheima`، وحاجتين كانوا فجوة حقيقية مش كود ميت فعلاً: شاشة الاستقبال ناقصاها
كارتين إحصائيات (غرف محجوزة/تحت التنظيف) محسوبين بالفعل بس معروضينش، ودالة في شاشة المرتبات كانت
كود ميت حقيقي (القيمة بقت بتتعرض سطر-بسطر بدل إجمالي واحد). حذفت `ProjectCockpitView.vue`
(أداة dev داخلية، صفر استخدام إنتاجي، نسخة احتياطية في `backups/deleted-features/`).

نفس المراجعة اتعملت لـ`elkheima-marketing-website` (وكيل تاني) — لقى نفس نمط الكود الميت
(40+ حالة)، وترك 3 حالات فجوة حقيقية مش كود ميت (`pt` غير مفعّل في 6 صفحات — نص قابل للتعديل من
لوحة تحكم موجود في الرئيسية بس مش الباقي، `minDate` في صفحتين مفيش فيهم حقل تاريخ أصلاً، زرار
"اطلب تاني" في DigitalHub مش موصول) — الثلاثة اتوثّقوا كقرارات محتوى محتاجة رأي محمد، مش كود.

**Gate 8 guest alerts (نداء الويتر/الحساب من QR) — اتأكد منه حي بالكامل ولقيت وصلحت باج حقيقي**:
الباك إند كان جاهز فعلاً ومؤمّن (session-derived location، idempotency/dedup، بث WebSocket حي،
TTL cleanup، 30 تست عادي) بس مقفول عمدًا خلف سويتشين (`GUEST_ALERTS_ENABLED` + `core.Setting`
لكل فرع) لحد ما حد يتأكد حي. عملت تجربة حية كاملة من الصفر: توكن QR حقيقي لطاولة → جلسة ضيف →
نداء ويتر → استلام المدير → acknowledge — كله عدّى عبر الـAPI مباشرة. لما جربت من متصفح حقيقي
(مش curl) لقيت باج حقيقي: الـتور التعليمي (`TourOverlay.vue`) كان بيغطي زرار "نداء ويتر" فعليًا
ويمنع الضغط عليه — السبب إن `fromQR` (بيحدد هل الضيف جاله السياق من QR حقيقي ولا لأ) كان لسه
بيفحص طريقة query params القديمة (`?type=&id=`) بس، مش مسار `/hub/:token` الجديد اللي النظام كله
اتنقل ليه. اتصلح في 3 أماكن، واتأكدت الرحلة الكاملة تاني من متصفح حقيقي بعد الإصلاح — نجحت،
لقطة شاشة حقيقية للزرار وهو بيتحول لـ"في الطريق! 🧑‍🍳". **لسه مقفولة على الإنتاج عمدًا** — قرار
تفعيل لمحمد، مش قرار تقني بعد كل ده.

**الشات بوت**: فحصت السويتشات التلاتة المطلوبة على الإنتاج فعليًا — `GEMINI_API_KEY` ✅،
`CHAT_PUBLIC_HOST_BRANCH_MAP` ✅ (اتصلح فوق نفس اليوم)، **`CHAT_PROVIDER_DATA_GOVERNANCE_VERIFIED`
لسه مش موجود** — ده مش حاجة أقدر أتحقق منها أو أفعّلها بنفسي، لازم محمد (أو مالك حساب Gemini/
Google الفعلي، بما إن المفتاح مشترك مؤقت من مشروع قديم) يدخل فعليًا على إعدادات الـproject في
Google AI Studio/Cloud Console ويتأكد إعدادات الـlogging/data-sharing مناسبة لبيانات ضيوف حقيقية،
وبعدها يقولّي أتفعّل السويتش. تفعيله بنفسي من غير التحقق ده هيبقى ادّعاء كذب.

**أول commit/push حقيقي**: بعد أسابيع من الشغل المتراكم (مني ومن Codex) من غير أي commit —
فحصت الـdiff بالكامل (مفيش أسرار، مفيش ملفات `.env` حقيقية، migrations الجديدة (3) طلعت متطبّقة
على الإنتاج بالفعل من نشر أمس فمفيش خطر migration جديد) وعملت commit + push حقيقي لـ`resort-os`
(167 ملف) و`elkheima-marketing-website` (109 ملف) على GitHub. الديبلوي بعد كده كان تحديث كود
مستهدف (`sync-deploy.sh`) لـ`backend`/`el_kheima`/`marketing_site` الثلاثة — مش full `deploy.sh`
لأن الـmigrations مكنتش جديدة فعليًا.

**متبقي معروف**:
- **قرار: تفعيل Gate 8 guest alerts على الإنتاج؟** — الميزة جاهزة ومؤكدة حية، القرار لمحمد.
- **قرار: تفعيل الشات بوت؟** — محتاج تحقق بشري من إعدادات Gemini الفعلية أولاً (فوق).
- فجوات المحتوى التلاتة في `elkheima-marketing-website` (`pt`/`minDate`/زرار اطلب تاني) —
  قرارات محتوى/تصميم لمحمد، مش كود.
- باقي القايمة المؤجَّلة من نشر أمس (نسخ احتياطي خارج السيرفر، مراقبة فعلية، UAT) — لسه زي ما هي.
...
## 🟩 طاولات الدايننج بقت مشتركة بين كل المنافذ (2026-07-21)

باج معماري حقيقي اكتشفه Mohamed من الاستخدام الفعلي: شاشة الكاشير الموحّدة
(`UnifiedPOSView.vue`) كانت بتعرض شبكة طاولات **مختلفة تمامًا** لما الكاشير
يبدّل بين المطعم والكافيه (مثلاً كافيه 1-8، مطعم 1-13 بترقيم منفصل) — عكس
الواقع الفعلي، اللي فيه نفس الصالة الفعلية بتخدم المطعم والكافيه بنفس
الطاولات بالظبط. السبب: `VenueTable.outlet_id` كان FK إجباري (الطاولة ملك
لمنفذ واحد بس)، فـ `_seed_dining_tables` كان بيزرع مجموعتين مكررتين من
الأساس.

**الإصلاح (فرع `dining-shared-tables`)**: الطاولة بقت ملك للفرع مش للمنفذ —
`outlet_id` اتشال من `VenueTable` بالكامل (migration `9b4e1a2c7f30`، بتدمج
أي فروع مكررة موجودة فعليًا لكل فرع مع حارس أمان يرفض يكمل لو فيه طلب نشط
على طاولة هتتحذف بدل ما يكسرها بصمت — اتأكد الاتنين المسارين حقيقي على
نسخة معزولة من قاعدة البيانات، مش الداتابيز المشتركة). تبديل المنفذ في
شاشة الكاشير والإدارة (`DiningMenuView.vue`) وشاشة QR
(`QRGeneratorView.vue`) بقى **يغيّر المنيو بس، مش شبكة الطاولات** — والطاولة
المختارة بقت تفضل زي ما هي بعد التبديل (كانت بتتصفّر). إضافة UX: طاولة
مشغولة بطلب من منفذ تاني (مثلاً كافيه وأنت واقف على تاب المطعم) بقى عليها
شارة واضحة توضح إن الطلب من منفذ تاني، بدل ما تفتكرها فاضية. تابات المنفذ
في الكاشير بقت بدل dropdown (تبديل أسرع وقت الزحمة).

**حماية أمان اتراجعت بالتوازي**: `GET /dining/public/menu`/`POST
/dining/public/orders` (طلب الضيف عبر QR) كانت بترفض `table_id` من منفذ
تاني — دي بقت مقبولة (الطاولة مشتركة فعلاً)، بس التحقق من إن الطاولة تتبع
**نفس الفرع** فضل قايم زي ما هو (الحماية الحقيقية المتبقية) — 4 اختبارات
جديدة بتغطي الحالتين (نفس الفرع مقبول، فرع تاني مرفوض).

**مؤجَّل عمدًا (Phase 2، محتاج مراجعة مستقلة منفصلة)**: طلب واحد فيه أصناف
من الكافيه والمطعم مع بعض على نفس الفاتورة — النظام المالي الحالي (Gate 4)
بيرحّل إيراد كل طلب لحساب واحد حسب منفذ الطلب ككل، مش لكل صنف، فلازم تعديل
منطق التسوية (settlement) قبل ما يتفتح ده. تفاصيل الخيارات في نقاش الشات.

53 backend test اتحدّثت/اتضافت (منها 4 اختبارات جديدة للحماية بعد التخفيف)،
`pytest tests/ -v` → **2010 passed, 33 skipped, صفر فشل**. frontend
type-check/build/vitest (60/60) نضاف. `git diff --check` نضيف. الفرع لسه
مش مدموج/مش push — بانتظار مراجعتك.

---

## 🟩 Gate 5 Batch 15 (الأخيرة) — Login/Auth ar/en + إغلاق Gate 5 (2026-07-21)

**الحالة:** آخر دفعة في مجهود ترجمة تطبيق الموظفين (`el-kheima`) الكامل
عربي/إنجليزي (Gate 5)، شغالة على فرع مستقل
`gate-5-staff-ux-batch-15-login-auth-i18n` متفرّع من نفس قاعدة Gate 4 (فرع
شقيق لدفعات 1-14). بلا commit/push خارج الفرع.

**اكتشاف مهم قبل البدء**: الخمس شاشات المستهدفة
(`account/LoginView.vue`, `ForgotPasswordView.vue`, `ResetPasswordView.vue`,
`ForcePasswordChangeView.vue`, `TwoFactorSetupView.vue`) كانت **مترجمة
بالكامل بالفعل** — بُنيت من الأساس بـ `useI18n()` وقت شغل الـ 2FA الإجباري
(Gate 2B2/2B3) لأنها جزء من سلسلة المصادقة نفسها. الدفعة دي بقت تحقق وترقية
لـ STRICT_FILES بدل ترجمة فعلية من الصفر.

**باج حقيقي اتكشف واتصلح رغم كده**: `ResetPasswordView.vue` كانت بتستخدم
`t('errors.generic')` كـ fallback لرسالة خطأ (لو رد الباك إند مش نص واضح)،
لكن المفتاح ده **مكانش موجود خالص** في `ar.json` ولا `en.json` — يعني لو
المستخدم واجه الحالة دي (خطأ 400 مش عن التوكن ولا عن قوة الباسورد)، كان
هيشوف حرفيًا النص `errors.generic` على الشاشة بدل رسالة مفهومة. اتصلح
بإضافة `errors.generic` لكلا الملفين تحت الـ namespace `errors` الموجود.

**تحقق شامل قبل الـ commit (على فرع الدفعة لوحده)**: `node
scripts/validate-i18n.mjs` نظيف (3116 مفتاح ar/en، صفر ناقص، صفر مخالفة
اتجاه فيزيائي، 12 STRICT_FILES)، `pnpm run type-check:all` نظيف، `npx
vitest run` → 60/60، `pnpm run build:all` نظيف لـ `el-kheima` و`public`،
`git diff --check` نظيف.

**Gate 5 كامل — كل 15 دفعة منفَّذة.** الدمج الفعلي لكل الـ 15 فرع في فرع
تكامل واحد (`gate-5-integration`) بدأ فعليًا بعد هذه الدفعة مباشرة — راجع
أعلى الملف/آخر commit للحالة النهائية المدموجة.
...
**الحالة:** جزء من مجهود ترجمة تطبيق الموظفين (`el-kheima`) الكامل عربي/إنجليزي
(Gate 5)، شغال على فرع مستقل `gate-5-staff-ux-batch-10-dining-recipes-foodcost-i18n`
متفرّع من نفس قاعدة Gate 4 (زي كل دفعات Gate 5 — فروع شقيقة منفصلة، مش متسلسلة
فوق بعض، لسه محتاجة دمج لاحقًا). بلا commit/push خارج الفرع — بانتظار الدمج
النهائي لكل دفعات Gate 5.

**الشاشات الثلاث المترجمة بالكامل** (أصبحت STRICT_FILES في
`validate-i18n.mjs` — ar/en كامل، مفيش `dir="rtl"` ثابت، مفيش `ar-EG`، مفيش
CSS اتجاهي فيزيائي):
- `DiningMenuView.vue` (951 سطر، 4 تابات: منافذ/منيو/طاولات/شاشات KDS + 4
  أدراج) — أول شاشة Gate 5 تستخدم DataTable columns محسوبة (`itemColumns`
  بقت `computed` بدل `const` عشان تتفاعل مع تغيير اللغة).
- `RecipesView.vue` (575 سطر) — وصفات/BOM + متغيّرات (حجم/نوع).
- `FoodCostReportView.vue` (347 سطر) — تقرير تكلفة الطعام، KPIs + رسم اتجاه
  يومي + جدول تفصيلي.

**باج حقيقي اتكشف واتصلح أثناء الترجمة، مش نسخ بس**: سكريبت Python للاستبدال
الجماعي لرسائل toast في `DiningMenuView.vue` كان أعمى تمامًا عن نطاق متغيرات
TypeScript — `deleteTable(t: VenueTable)` كانت بتستخدم `t` كاسم بارامتر، فاستبدال
السكريبت لـ `'تم حذف الطاولة'` بـ `t('backoffice.diningMenu.msg.tableDeleted')`
كان هيولّد استدعاء لبارامتر `VenueTable` بدل دالة `t()` بتاعة vue-i18n — كان
هيرمي خطأ فعلي أول مرة موظف يحذف طاولة. اتكشف بمراجعة يدوية بعد تشغيل
السكريبت مباشرة (مش من فشل تست)، واتصلح بإعادة تسمية البارامتر لـ `tbl` في
كل الدوال المتأثرة. **الدرس المضاف للمنهجية**: أي سكريبت استبدال جماعي لازم
تتبعه مراجعة يدوية لتصادمات `t` في نفس الملف قبل الانتقال للخطوة التالية.

**تحقق شامل قبل الـ commit**: `node scripts/validate-i18n.mjs` نظيف (3396
مفتاح ar/en، صفر ناقص، صفر مخالفة اتجاه فيزيائي)، `pnpm run type-check:all`
نظيف (اكتشف باج تايبسكريبت حقيقي: `itemColumns`/`STATIONS` بقوا `computed`
...
## 🟩 Gate 5 — اكتمال إدارة الموظفين بلغتين — Batch 9: المالية (2026-07-20)

**الحالة:** منفَّذة ومُتحقَّق منها ذاتيًا — نُفِّذت مباشرة من غير وكيل، على فرع
مستقل `gate-5-staff-ux-batch-9-finance-i18n`. **بلا commit/push بعد.** شاشة
واحدة (`FinanceView.vue`) لكنها أكبر شاشة اتترجمت لحد دلوقتي — 8 تابات
(نظرة عامة، الشيكات، الحسابات، مراكز التكلفة، الميزانية العمومية، إهلاك
الأصول، التسوية البنكية، الورديات) + مودال تفاصيل وردية كامل. بقت
`STRICT_FILES`.

**تنظيف حقيقي اتكشف أثناء الترجمة:** نفس فئة مخالفات Gate 3 — `dir="rtl"`
ثابت في مكانين (الجذر + مودال تفاصيل الوردية)، و`ar-EG`
locale calls (`toLocaleString`/`toLocaleDateString`) اتصلحت
بـ`useStaffFormat()`. فاحص `validate:i18n` كشف استخدامين فيزيائيين
(`mr-1`، `mr-2`) — اتصلحوا لـ`ms-1`/`ms-2`. parameter اسمه `t`
(`loadTab(t: typeof tab.value)`) كان بيغطّي دالة الترجمة — اتغيّر
لـ`tabId`. 5 خرائط تصنيف/تسمية (METHOD_LABEL، checkStatusConfig،
statementLineStatusConfig...) اتحوّلت من `const` لـ`computed`.

**مفاتيح جديدة:** `backoffice.finance.*` — 142 مفتاح (استُخدمت جميعها، صفر
ناقص، صفر زيادة).

**التحقق:** `validate:i18n` أخضر (3257 مفتاح ar/en متطابق، صفر ناقص، صفر
مخالفة اتجاه فيزيائي)، `type-check:all` (el-kheima + public) نظيف،
`test:frontend` 60/60 (صفر رجوع)، `build:all` نظيف. `git diff --check` نظيف.

**الباقي (~31 شاشة admin/ops/portal):** لسه مؤجَّل لدفعات لاحقة — التالية
حسب الخطة: دايننج/وصفات/تكلفة (Batch 10).
...
**الحالة:** منفَّذة ومُتحقَّق منها ذاتيًا — نُفِّذت مباشرة من غير وكيل، على فرع
مستقل `gate-5-staff-ux-batch-7-beach-admin-i18n`. **بلا commit/push بعد.**
شاشة واحدة: `BeachAdminView.vue` (4 تابات: الملخص اليومي، المعاملات، عقود
B2B، تقرير نهاية اليوم — زائد 4 مودالات: Surge، عقد B2B جديد، تسوية، تعديل
ائتمان). بقت `STRICT_FILES`.

**تنظيف حقيقي اتكشف أثناء الترجمة:** `dir="rtl"` ثابت، وparameter اسمه `t`
(`switchTab(t: typeof tab.value)`) كان بيغطّي دالة الترجمة — اتغيّر
لـ`tabId`. `txTypeLabels` (تصنيفات نوع المعاملة) اتحوّلت من `const` ثابت
لـ`computed`.

**مفاتيح جديدة:** `backoffice.beachAdmin.*` — 105 مفتاح (استُخدمت جميعها،
صفر ناقص، صفر زيادة).

**التحقق:** `validate:i18n` أخضر (3220 مفتاح ar/en متطابق، صفر ناقص، صفر
مخالفة اتجاه فيزيائي)، `type-check:all` (el-kheima + public) نظيف،
`test:frontend` 60/60 (صفر رجوع)، `build:all` نظيف. `git diff --check` نظيف.

**الباقي (~34 شاشة admin/ops/portal):** لسه مؤجَّل لدفعات لاحقة — التالية
حسب الخطة: خريطة/POS الشاطئ (Batch 8).
...
## 🟩 Gate 5 — اكتمال إدارة الموظفين بلغتين — Batch 5: استقبال + حجوزات (2026-07-20)

**الحالة:** منفَّذة ومُتحقَّق منها ذاتيًا — نُفِّذت مباشرة من غير وكيل، على فرع
مستقل `gate-5-staff-ux-batch-5-reception-bookings-i18n`. **بلا commit/push
بعد.** شاشتان: `ReceptionView.vue` (لوحة الاستقبال الرئيسية — وصولات، خريطة
غرف، تنظيف معلق، تسجيل دخول/خروج، حجز جديد، Night Audit) و`BookingsView.vue`
(قائمة الحجوزات الكاملة — فلترة بالحالة، وصول/خروج مبكر ومتأخر، إلغاء حجز).
الاتنين بقوا `STRICT_FILES` في `validate-i18n.mjs`.

**تنظيف حقيقي اتكشف أثناء الترجمة:** نفس فئة مخالفات Gate 3 — `dir="rtl"`
ثابت في الشاشتين، و`ar-EG`/`toLocaleTimeString`/`toLocaleDateString` بدل
الفورمات المركزية، اتصلحوا بـ`useStaffFormat()`. فاحص `validate:i18n`
الصارم كشف استخدامين فعليين لـ`text-right`/`mr-1` في `BookingsView.vue` —
اتصلحوا لـ`text-start`/`ms-1`. فاصل قائمة أسماء الغرف
(`roomName`/`join('، ')`) كان فاصل عربي ثابت — بقى مفتاح ترجمة يتغيّر مع
اللغة بدل نص حرفي.

**مفاتيح جديدة:** `backoffice.reception.*` (76 مفتاح) + `backoffice.bookings.*`
(70 مفتاح) — استُخدمت جميعها، صفر ناقص، صفر زيادة.

**التحقق:** `validate:i18n` أخضر (3261 مفتاح ar/en متطابق، صفر ناقص، صفر
مخالفة اتجاه فيزيائي)، `type-check:all` (el-kheima + public) نظيف،
`test:frontend` 60/60 (صفر رجوع)، `build:all` نظيف. `git diff --check` نظيف.

**الباقي (~37 شاشة admin/ops/portal):** لسه مؤجَّل لدفعات لاحقة — التالية
حسب الخطة المتفق عليها: غرف + تدبير منزلي (Batch 6).

## 🟩 Gate 5 — اكتمال إدارة الموظفين بلغتين — Batch 4: HR (2026-07-20)

**الحالة:** منفَّذة ومُتحقَّق منها ذاتيًا — نُفِّذت مباشرة من غير وكيل، على فرع
مستقل `gate-5-staff-ux-batch-4-hr-i18n`. **بلا commit/push بعد.** شاشة واحدة
(`HRView.vue`) — 5 تابات (الموظفون، الحضور، الرواتب، الإجازات، لوحة الأداء)
+ 6 مودالات (بدلات، جزاءات، راتب/تأمين/مكافأة، سلف، دفعات، رصيد إجازة،
استيراد حضور، تصحيح حضور). بقت `STRICT_FILES` في `validate-i18n.mjs`.

**تنظيف حقيقي اتكشف أثناء الترجمة (مش بس نص):** نفس فئة مخالفات Gate 3 —
`dir="rtl"` ثابت و`.toLocaleDateString('ar-EG')`/`.toLocaleString('ar-EG')`
بدل الفورمات المركزية، اتصلحوا بـ`useStaffFormat()`. فاحص `validate:i18n`
الصارم كشف 3 مواضع physical CSS (`text-right`، `mr-2`، `text-left`،
`file:ml-3`) — اتصلحوا للمنطقية المقابلة. كان فيه كمان function parameter
اسمه `t` (`function loadTab(t: typeof tab.value)`) ومتغيّر حلقة `v-for="t
in [...]"` بيغطّوا دالة الترجمة `t()`، زائد `const t = ...find(...)` جوه
`onPenaltyTypeChange` — اتغيّر اسمهم كلهم. خريطة تصنيفات الحالة
(`statusLabels`) اتحوّلت من `const` ثابت لـ`computed`.

**مفاتيح جديدة:** `backoffice.hr.*` — 184 مفتاح (استُخدمت جميعها، صفر ناقص،
صفر زيادة).

**التحقق:** `validate:i18n` أخضر (3299 مفتاح ar/en متطابق، صفر ناقص، صفر
مخالفة اتجاه فيزيائي)، `type-check:all` (el-kheima + public) نظيف،
`test:frontend` 60/60 (صفر رجوع)، `build:all` نظيف. `git diff --check` نظيف.

**الباقي (~38 شاشة admin/ops/portal):** لسه مؤجَّل لدفعات لاحقة.

## 🟩 Gate 5 — اكتمال إدارة الموظفين بلغتين — Batch 3: CRM (2026-07-20)

**الحالة:** منفَّذة ومُتحقَّق منها ذاتيًا — نُفِّذت مباشرة من غير وكيل، على فرع
مستقل `gate-5-staff-ux-batch-3-crm-i18n`. **بلا commit/push بعد.** شاشة واحدة
بس (`CRMView.vue`) لكنها أكبر شاشة اتترجمت لحد دلوقتي — 7 تابات (عملاء محتملون،
عملاء، فرص بيعية، أنشطة، حملات، ملفات ضيوف، برنامج ولاء) + مودالين (تفاصيل
عميل محتمل، مجموعات العملاء). بقت `STRICT_FILES` في `validate-i18n.mjs`.

**تنظيف حقيقي اتكشف أثناء الترجمة (مش بس نص):** نفس فئة مخالفات Gate 3 اللي
اتكشفت في Batch 1/2 — `dir="rtl"` ثابت على الجذر و`.toLocaleDateString('ar-EG')`/
`.toLocaleString('ar-EG')` بدل الفورمات المركزية، اتصلحوا بـ`useStaffFormat()`.
فاحص `validate:i18n` الصارم كشف استخدامين فعليين لـ`text-right`/`mr-2`
(physical CSS) — اتصلحوا لـ`text-start`/`ms-2` المنطقية. كان فيه كمان
باراميتر function واسمه `t` (`function loadTab(t: typeof tab.value)`) ومتغيّر
حلقة `v-for="t in [...]"` بيغطّوا دالة الترجمة `t()` بتاعة vue-i18n — اتغيّر
اسمهم. كل الـ label/config lookup maps (مراحل العميل المحتمل، تصنيفات
الاهتمام/النتيجة/الحملة...) اتحوّلوا من `const` ثابت لـ `computed` عشان يعيدوا
الحساب لو اللغة اتغيّرت. حجم غير عادي من رسائل toast (55 رسالة) اتستبدلت
بسكريبت Python بديل بدل تعديل يدوي واحدة واحدة.

**مفاتيح جديدة:** `backoffice.crm.*` — 241 مفتاح (استُخدمت جميعها، صفر ناقص،
صفر زيادة).

**التحقق:** `validate:i18n` أخضر (3356 مفتاح ar/en متطابق، صفر ناقص، صفر
مخالفة اتجاه فيزيائي)، `type-check:all` (el-kheima + public) نظيف،
`test:frontend` 60/60 (صفر رجوع)، `build:all` نظيف. `git diff --check` نظيف.

**الباقي (~39 شاشة admin/ops/portal):** لسه مؤجَّل لدفعات لاحقة — HR
(Batch 4) هي التالية حسب طلب Mohamed الصريح.
