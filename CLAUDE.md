...
☐ Error handling كامل
☐ لا N+1 queries
☐ pytest tests/ -q لا يزال يعطي 734 passed
...
4. معظم شاشات الفرونت إند بتستخدم `axios` مباشرة بدل `@resort-os/core`'s `api` client المُعدّ مسبقاً —
   يعني الـ 401-interceptor (auto-logout + redirect) مش شغال إلا في شاشة واحدة بس. إصلاحه محتاج مراجعة
   أكتر من 25 ملف بحرص شديد، مش patch سريع.
5. "الدفع على حساب الغرفة" (Charge to Room) — حجوزات PMS بترحّل إيرادها للدفتر مباشرة عند checkout
   (Dr.Cash/Cr.Revenue، زي كل موديول تاني)، لكن مفيش Folio بيربط ضيف مقيم بمشترياته من موديولات تانية
   (مطعم/شاطئ/كافيه) عشان يتحاسب عليها كلها مع بعض وقت الخروج — قرار تصميم أكبر لسه محتاج يتاخد.
6. تكامل تنبيهات واتساب/SMS — أكتر من 9 أماكن (Celery tasks) فيها "TODO" بدل إرسال فعلي.

### 🟢 تحسينات مستقبلية
7. بيانات CRM (`total_spent`/`visits_count`) — الدالة اللي بتحدّثهم موجودة وشغالة بس محدّش بينادي
   عليها، لأن مفيش موديول تاني فيه `customer_id` يربط الطلب بعميل CRM أصلاً.
8. تكرار كود ترحيل الإيرادات (Dr.Cash/Cr.Revenue) عبر 6 موديولات — يستاهل دالة مشتركة، مش عاجل.
9. Screen-level permissions (CanShow/CanAdd/CanEdit/CanDelete) — فيه أساس (`UserPermission`) بس مش
   كامل التغطية.
10. Auto-posting (كل فاتورة → journal entry تلقائي) خارج المسارات اللي بتعمل كده أصلاً.
...
# Tests (لا تكمل أي task قبل ما 734 test تعدي)
