...
> المناسبة للمهمة.
>
> **مصدر التنفيذ الحالي — 30 يوليو 2026:** بعد القراءة الإلزامية أعلاه، اقرأ
> `docs/README.md` ثم الخطة النهائية ولوحة التنفيذ المشار إليهما داخله.
> أحدث handoff في اللوحة هو مصدر حالة الـVPS غير المرئية في Git. Codex هو
> قائد التنفيذ والمراجع النهائي بتكليف Mohamed. أي ملف داخل `docs/archive/`
> تاريخي وممنوع تنفيذ أوامر commit/deploy/DNS منه.
...
الملكية الجزئية، رسوم الغرف، حركة المخزون، الرواتب، تكاليف الصيانة — كل ده لازم يفضل قابل
...
# في 6 موديولات مختلفة (KDS، PMS، ملكية جزئية، موارد بشرية، إيجارات، شاطئ)
...
# اتاخد صح (اتكشف في beach.crud وملكية جزئية 2026-07-06؛ SQLite بيتجاهل
...
- **استبيان رضا الملكية الجزئية بقى قابل للاستخدام فعليًا** (2026-07-06) — `SurveyView.vue` والباك إند
  ...
  الملكية الجزئية لكل زيارة `completed`. `SurveyView.vue` نفسها بقت adaptive: بتقرا `ref_type` من الـ
  JWT payload (بدون تحقق توقيع، للعرض بس — التحقق الحقيقي سيرفر-سايد) وتعرض فئات تقييم خاصة
  بالملكية الجزئية (نظافة الوحدة/الاستقبال/نظافة الشاطئ/توافر المرافق، مأخوذة من نموذج استطلاع ورقي حقيقي)
...
- **`chat` — تحصين CL-01 حسب بروتوكول Codex/Claude المزدوج** (2026-07-26، نفس اليوم، دفعة تانية) —
  Mohamed بعتلي برومت رسمي لتشغيل بروتوكول التنفيذ المزدوج (مؤرشف الآن تحت
  `docs/archive/2026-07-execution/`، مع الخطة القديمة)؛ كتبهم
...
- **حساب إيراد الغرف الموحّد (القرار المؤجَّل من 2026-07-07) — التصميم اتنفّذ فعليًا 2026-07-25/26،
  لكن دليل الحسابات كان فاضي بالكامل على الإنتاج الحقيقي لحد 2026-07-27** — فجوتين مختلفتين اتكشفوا في
  نفس المراجعة:
  1. **الكود نفسه صحيح ومطابق لقرار Mohamed تمامًا**: `run_night_audit`
     (`pms/services.py::_post_room_revenue_for_night_audit`) بيرحّل إيراد الحجوزات العادية يوميًا
     Dr.1150/Cr.**4100**، وعقود الملكية الجزئية بترحّل لـ**4600**/**4650** (`timeshare/services.py`) —
     كل الحسابات التلاتة بنات لحساب أب واحد **4000 الإيرادات** (`Account.parent_id` حقيقي، مش تسمية
     بس)، بالظبط التمييز اللي Mohamed طلبه (ملكية جزئية مقابل حجز فندقي عادي تحت مظلة واحدة).
     `_post_checkout_journal` بقى بيسوّي الذمة بس (Dr.نقدية/Cr.1150) — مش بيسجّل إيراد مباشر، الإيراد
     الحقيقي بيتسجّل يوميًا في Night Audit زي أي PMS حقيقي (Opera/Mews/Cloudbeds).
  2. **اكتشاف حرج منفصل أثناء التحقق الحي (2026-07-27)**: `python -m app.seed` (اللي بيزرع دليل
     الحسابات، `_seed_chart_of_accounts`) ممنوع صراحةً من الشغل على production بتصميم — وده معناه
     دليل الحسابات **مكانش موجود خالص** على قاعدة بيانات الإنتاج الحقيقية (اتأكد بـSQL مباشر: 0 حساب،
     0 قيد يومية من أول ما السيرفر اشتغل). وبما إن `post_simple_revenue_journal` (الدالة المشتركة
     لكل الـ6 موديولات) مصمّمة عمدًا تبتلع أي خطأ ترحيل (زي "حساب مش موجود") وترجع `None` بدل ما توقف
     العملية التشغيلية الحقيقية — **كل عملية بيع حقيقية حصلت فعليًا على الإنتاج (شاطئ/مطعم/كافيه/PMS)
     كانت بتنجح تشغيليًا 100% لكن بصفر أثر محاسبي خالص**، بصمت، من غير أي خطأ ظاهر لحد ما.
  **الحل المنفَّذ (بموافقة Mohamed الصريحة 2026-07-27)**: `_seed_chart_of_accounts` اتشغّلت مباشرة ضد
  production (فنكشن معزولة idempotent، 28 حساب حقيقي بس — بدون أي بيانات تجريبية/وهمية من باقي
  `app.seed`)، واتأكد الترحيل شغال حي (نداء حقيقي لـ`post_simple_revenue_journal` رجع `JournalEntry`
  حقيقي، بعدين `rollback()` عشان مايسيبش أثر). **قرار موثّق صراحةً**: مفيش backfill رجعي للمعاملات
  القديمة اللي فاتت من غير قيد — نفس سبب قرار عدم الـbackfill لـ`cost_center_id` قبل كده (تعقيد غير
  مبرر لبيانات قبل الإصلاح)، الإصلاح سارٍ على أي معاملة جديدة من دلوقتي بس.
- **عقد أسعار Dining النهائي** (قرار Mohamed الصريح 2026-08-12) — كل سعر
  منشور في منيو الخيمة سعر نهائي يشمل VAT والخدمة. `DiningItem.price` يظل
  الرقم الظاهر في الموقع والـPOS، ولا تُضاف عليه الضريبة والخدمة مرة ثانية
  عند التحصيل. الطلب يحفظ لقطة السعر النهائي مع لقطة صافي منفصلة حتى تبقى
  القيود والتقارير قادرة على فصل الإيراد والضريبة والخدمة. لقطات الطلبات
  التاريخية لا تتغير؛ `listed_* = NULL` يحدد عقد السعر الصافي القديم.
- **إصلاح جذري لإيصالات/فواتير PDF العربية + لوجو + تصميم احترافي** (2026-08-04) — Mohamed رفع سكرين
  شوت لفاتورة أوردر حقيقية: كل النص العربي فيها ظاهر كمربعات سودة صلبة تمامًا. تشخيص كامل كشف 3 باجات
  متسلسلة في `app.core.kernel.reports.ReportBuilder` (كانت موجودة من أول ما الملف اتعمل، مش تراجع):
  1. **مفيش أي خط TTF متسجّل في المشروع خالص** — كل الرسم كان بيستخدم `"Helvetica"`/`"Helvetica-Bold"`
     (base-14 فونتات ReportLab، صفر glyphs عربي) رغم إن `_t()` كانت بترتّب/تعيد تشكيل النص العربي صح
     منطقيًا — المشكلة كانت في الرسم مش في التجهيز.
  2. `NotoSansArabic-Regular/Bold.ttf` (الخط المتاح محليًا، حزمة Debian `fonts-noto-core`) لما اتسجّل
     لوحده، الخط ده **مفهوش أي glyph لاتيني خالص** (اتأكد بـ`fontTools.ttLib.TTFont.getBestCmap()`) —
     يعني تسجيله لوحده كان هيصلح العربي ويخفي كل كلمة إنجليزية/رقم لاتيني تمامًا (مش تربيعة، اختفاء
     فعلي). الحل: `_split_script_runs()` + `ReportBuilder._draw_mixed()` جديدين — بيقسّموا أي سطر نص
     (بعد التشكيل/الـbidi) لأجزاء حسب السكريبت الفعلي، وبيرسموا كل جزء بالخط اللي بيغطّيه فعليًا
     (عربي = NotoSansArabic، لاتيني = Helvetica) بدل خط واحد ثابت لكل السطر.
  3. `_add_footer` كانت `@staticmethod` بترسم الـtext مباشرة من غير ما تنادي `_t()` خالص — يعني تذييل
     الإيصال (رقم الفرع/الشكر) كان ظاهر بعربي مش متشكّل ومش bidi، حتى بعد إصلاح البندين فوق. اتحوّلت
     لـinstance method بتنادي `self._draw_mixed(...)` زي كل مكان تاني.
  اتضاف كمان: لوجو المنتجع الحقيقي (`app/assets/logo.png`، منسوخ من شعار الفرونت إند) مرسوم في رأس كل
  إيصال حراري (كان غير موجود خالص قبل كده)، وتصميم أرقى للإيصال الحراري نفسه (فواصل متقطّعة، تفصيل
  السعر تحت كل صنف، قسم الإجمالي منفصل بصريًا، إجمالي كبير بلون مميز) عبر `receipt_pdf_thermal`
  الجديدة (باراميترز `items`/`summary`/`subtitle`/`show_logo` اختيارية — باقي المستهلكين
  beach/leasing/timeshare/hr مكملين بنفس الشكل القديم بالظبط، مفيش breaking change). الفونتات
  والصورة اتحطّوا في `backend/app/assets/` (مش مسار نظام) عشان `Dockerfile`'s `COPY . /app` ينقلهم
  للإنتاج فعليًا — الـ`python:3.11-slim` النهائي مفهوش فونتات نظام خالص. `dining.services.
  generate_receipt_pdf` اتعمله rewrite كامل يستخدم الشكل الجديد الغني. اتأكد بـ2 رندر PDF حقيقي كامل
  (مش افتراض) + `pytest tests/ -k "receipt or report or payslip or pdf"` أخضر بالكامل قبل ما يتقفل.
...
### 🟢 تحسينات مستقبلية
6. تكرار كود ترحيل الإيرادات — اتحلّ جزئيًا (دالة مشتركة موجودة ومستخدمة في الـ 6 موديولات).
   ~~مفيش معاملة حقيقية live بعملة غير الجنيه (المسار مجرّب بالتست بس)~~ — **اتصلح في POS-03/POS-03b
   (2026-08-05)**: المطعم/الكافيه والشاطئ بقوا يقبلوا كاش USD/EUR حقيقي مع تسجيل currency/fx_rate
   على Payment وتقرير variance لكل عملة في نهاية الوردية.
