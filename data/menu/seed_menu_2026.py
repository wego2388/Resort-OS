"""
data/menu/seed_menu_2026.py
────────────────────────────────────────────────────────────────────────
جاهز للنسخ إلى backend/app/ وتشغيله منفصلاً عن seed.py الأصلي.

يعمل upsert آمن:
  - DiningItem موجود بنفس الاسم في نفس الـ outlet  → UPDATE السعر + name_ar
  - DiningItem غير موجود                            → INSERT جديد
  - صنف في الـ database لكن مش في المنيو الجديد     → is_available=False (لا حذف)

مبني على:
  - بنية seed.py الموجودة (نفس الـ helpers)
  - المنيو الجديد المستخرج من الصور (أغسطس 2026)
  - outlet_type='restaurant' + outlet_type='cafe' اللي موجودين
"""
...
def _upsert_item(db: Session, *, branch_id: int, outlet_id: int,
                 ...
                 available_note: str | None = None) -> None:
...
def _deactivate_removed(db: Session, outlet_id: int, active_names: set[str]) -> int:
    """items في الـ DB لكن مش في المنيو الجديد → is_available=False."""
    from app.modules.dining.models import DiningItem
...
def _update_restaurant_menu(db: Session) -> None:
    ...
    for sort_order, cat_en, name_en, name_ar, price, station, desc_en, desc_ar in ITEMS:
        ...
        cat_id = cat_map[cat_en]
        if name_en in existing:
            item = existing[name_en]
            item.price        = price
            item.name_ar      = name_ar
            ...
            item.description_ar = desc_ar
            updated += 1
        else:
            db.add(DiningItem(
                branch_id=branch.id, outlet_id=outlet.id,
                category_id=cat_id, name=name_en, name_ar=name_ar,
                price=price, station=station, sort_order=sort_order,
                is_available=True, description=desc_en, description_ar=desc_ar,
            ))
            inserted += 1

    deactivated = _deactivate_removed(db, outlet.id, active_names)
    db.flush()
    print(
...
def apply_translations(db: Session, json_path: str = "data/menu/menu_i18n_structure.json") -> None:
    """
    بعد ما تملى name_ru و name_it في menu_i18n_structure.json،
    شغّل الدالة دي عشان تكتب الترجمات في الـ database.
