...
export const CRITICAL_FINDINGS: CriticalFinding[] = [
  ...
  { id: 'superadmin', icon: 'shield-warning', severity: 'high', title: { ar: 'حماية Super Admin — الأساسيات مقفولة، step-up/TOTP باقيان', en: 'Super Admin safeguards — core invariants closed, step-up/TOTP remain' }, impact: { ar: 'Gate 2A أغلقت override/deny الذي يسقط super_admin نشط وقفل آخر مسؤول. باقي: change_password يتخطى كلمة السر الحالية لـadmin/super_admin، وTOTP وقت الدخول غير مفروضة فعليًا في الإنتاج.', en: 'Gate 2A closed overrides/denies that could drop an active super_admin or lock out the last administrator. Remaining: change_password bypasses the current-password check for admin/super_admin, and login-time TOTP is not enforced in production yet.' }, evidence: { ar: 'has_permission وupdate_user_role/grant_permission بقوا محميين ومختبرين (98 اختبار مستهدف + 2 Postgres حقيقي)؛ راجع docs/audits/gate-2a-super-admin-invariants.md.', en: 'has_permission and update_user_role/grant_permission are now protected and tested (98 targeted tests + 2 real Postgres); see docs/audits/gate-2a-super-admin-invariants.md.' }, action: { ar: 'Gate 2B: تخطيط أمني فقط لـchange_password step-up وTOTP وقت الدخول قبل أي تنفيذ.', en: 'Gate 2B: security-only planning for change_password step-up and login-time TOTP before any implementation.' }, source: 'docs/decisions/0003-super-admin-control-plane.md', scope: 'core', phase: 'superadmin-backend' },
...
export const READINESS_LENSES: ReadinessLens[] = [
  ...
  { id: 'authorization', icon: 'key', title: { ar: 'الصلاحيات والعزل', en: 'Authorization and isolation' }, state: 'risk', severity: 'high', finding: { ar: 'الأساس server-side موجود؛ Gate 2A أثبتت حماية Super Admin الأساسية (deny/override/last-admin). عزل النطاق (branch/outlet) لباقي endpoints لسه يحتاج إثباتًا.', en: 'Server-side foundations exist; Gate 2A proved the core Super Admin safeguards (deny/override/last-admin). Branch/outlet scope isolation for the remaining endpoints still needs proof.' }, evidence: { ar: 'Permission matrix مع override محمي لـsuper_admin؛ last-admin guard مُختبَر تحت تزامن حقيقي (docs/audits/gate-2a-super-admin-invariants.md).', en: 'Permission matrix with overrides now protected for super_admin; last-admin guard tested under real concurrency (docs/audits/gate-2a-super-admin-invariants.md).' }, nextGate: { ar: 'Gate 2B: step-up/TOTP، ثم عزل نطاق الفرع/المنفذ لباقي الـendpoints.', en: 'Gate 2B: step-up/TOTP, then branch/outlet scope isolation for the remaining endpoints.' }, scope: 'core', phase: 'superadmin-backend' },
...
export const SMART_EXECUTION_PHASES: SmartExecutionPhase[] = [
  ...
  { id: 'admin-security', order: 2, status: 'ready', title: { ar: 'أمان مركز التحكم — Gate 2A مُعتمَدة', en: 'Control-plane security — Gate 2A accepted' }, purpose: { ar: 'حماية Super Admin والجلسات وTOTP وstep-up والتدقيق.', en: 'Protect Super Admin, sessions, TOTP, step-up, and audit.' }, dependencies: { ar: 'Gate 2A (deny/override/self-lockout/last-admin) مُعتمَدة 2026-07-18؛ Gate 2B (step-up/TOTP) على فرع مستقل بعدها.', en: 'Gate 2A (deny/override/self-lockout/last-admin) accepted 2026-07-18; Gate 2B (step-up/TOTP) follows on its own branch.' }, exitGate: { ar: 'منع escalation/self-lockout بالتزامن في السيرفر — Gate 2A اجتازت هذا فعليًا (2/2 Postgres حقيقي).', en: 'Server-side escalation/self-lockout prevention under concurrency — Gate 2A already passed this (2/2 real Postgres).' } },
