...
export const CRITICAL_FINDINGS: CriticalFinding[] = [
  { id: 'finance-fail-open', icon: 'currency', severity: 'critical', title: { ar: 'آثار مالية تُبتلع أخطاؤها', en: 'Financial side-effect failures are swallowed' }, impact: { ar: 'قد تنجح عملية البيع أو الرواتب أو استهلاك المخزون بينما يفشل القيد أو الفوليو.', en: 'Sales, payroll, or stock consumption can succeed while journal or folio posting fails.' }, evidence: { ar: '`except Exception: pass/return None` في Finance وHR وDining وBeach وInventory.', en: '`except Exception: pass/return None` across Finance, HR, Dining, Beach, and Inventory.' }, action: { ar: 'صمم atomic/outbox contract واختبارات فشل قبل تغيير أي call site.', en: 'Design an atomic/outbox contract and failure tests before changing call sites.' }, source: 'docs/audits/PRODUCTION_READINESS_AUDIT.md#c-01--بعض-العمليات-المالية-fail-open-وليست-ذرية', scope: 'finance', phase: 'financial-atomicity' },
  { id: 'public-trust', icon: 'qrcode', severity: 'critical', title: { ar: 'Public/QR يثق في IDs ويتيح self-order', en: 'Public/QR trusts IDs and exposes self-ordering' }, impact: { ar: 'تخمين موقع أو طلب، spam، وإنشاء طلب ضيف يخالف view_and_call.', en: 'Location/order guessing, spam, and guest order creation that conflicts with view_and_call.' }, evidence: { ar: 'Dining public routes وGuestAlert schema وrate-limit map الحالية.', en: 'Current Dining public routes, GuestAlert schema, and rate-limit map.' }, action: { ar: 'تحقق من التعرض ثم containment صغير قبل بناء QR الكامل.', en: 'Verify exposure, then perform bounded containment before full QR work.' }, source: 'docs/audits/PRODUCTION_READINESS_AUDIT.md#c-02--حدود-الثقة-العامة-للـqr-والطلب-الذاتي-لا-تطابق-القرار-المعتمد', scope: 'dining', phase: 'public-exposure-containment' },
  { id: 'superadmin', icon: 'shield-warning', severity: 'high', title: { ar: 'حماية Super Admin غير مكتملة', en: 'Super Admin safeguards are incomplete' }, impact: { ar: 'يمكن override أو deny أو تعطيل يسبب تصعيدًا أو قفل آخر مسؤول.', en: 'Overrides, denies, or deactivation can cause escalation or lock out the last administrator.' }, evidence: { ar: 'سياسة has_permission وupdate_user_role/grant_permission الحالية.', en: 'Current has_permission and update_user_role/grant_permission behavior.' }, action: { ar: 'مرحلة backend مستقلة بعد البوابة الحرجة.', en: 'A dedicated backend phase after the critical safety gate.' }, source: 'docs/decisions/0003-super-admin-control-plane.md', scope: 'core', phase: 'superadmin-backend' },
...
export const READINESS_LENSES: ReadinessLens[] = [
  ...
  { id: 'database', icon: 'archive', title: { ar: 'Database وMigrations', en: 'Database and migrations' }, state: 'verified', severity: 'foundation', finding: { ar: 'head واحد والقاعدة المحلية current؛ المراجعة model-by-model لم تتم بعد.', en: 'One head and a current local database; model-by-model review remains.' }, evidence: { ar: 'Alembic heads/current = 9989c0432ccc.', en: 'Alembic heads/current = 9989c0432ccc.' }, nextGate: { ar: 'كل migration جديدة لها data impact وrollback وPostgreSQL test.', en: 'Every new migration needs data impact, rollback, and PostgreSQL tests.' }, scope: 'all', phase: 'readiness-baseline' },
  { id: 'finance', icon: 'currency', title: { ar: 'المال والمحاسبة', en: 'Finance and accounting' }, state: 'risk', severity: 'critical', finding: { ar: 'هناك fail-open بين العملية التشغيلية والقيد/الفوليو/COGS.', en: 'Fail-open behavior exists between operations and journal/folio/COGS effects.' }, evidence: { ar: 'خمسة call sites مثبتة في مراجعة 360°.', en: 'Five code-proven call sites in the 360° audit.' }, nextGate: { ar: 'Atomicity/reconciliation contract + failure injection.', en: 'Atomicity/reconciliation contract + failure injection.' }, scope: 'finance', phase: 'financial-atomicity' },
  { id: 'authorization', icon: 'key', title: { ar: 'الصلاحيات والعزل', en: 'Authorization and isolation' }, state: 'risk', severity: 'high', finding: { ar: 'الأساس server-side موجود؛ حماية Super Admin وعزل النطاق يحتاجان إثباتًا.', en: 'Server-side foundations exist; Super Admin safety and scope isolation need proof.' }, evidence: { ar: 'Permission matrix مع override؛ لا last-admin guard.', en: 'Permission matrix with overrides; no last-admin guard.' }, nextGate: { ar: 'اختبارات deny/escalation/branch-outlet isolation.', en: 'Deny, escalation, and branch/outlet isolation tests.' }, scope: 'core', phase: 'superadmin-backend' },
  { id: 'public-security', icon: 'qrcode', title: { ar: 'أمان Public وQR', en: 'Public and QR security' }, state: 'risk', severity: 'critical', finding: { ar: 'IDs متسلسلة وسياق من العميل وself-order عام وrate limits قديمة.', en: 'Sequential IDs, client context, public self-ordering, and stale rate limits.' }, evidence: { ar: 'Dining/public وGuestAlert وrate_limit.py.', en: 'Dining/public, GuestAlert, and rate_limit.py.' }, nextGate: { ar: 'Exposure check ثم containment؛ token architecture لاحقًا.', en: 'Exposure check then containment; token architecture later.' }, scope: 'dining', phase: 'public-exposure-containment' },
  ...
  { id: 'testing', icon: 'clipboard', title: { ar: 'Testing والجودة', en: 'Testing and quality' }, state: 'risk', severity: 'high', finding: { ar: 'Backend واسع؛ Frontend بلا tests/lint/a11y/E2E.', en: 'Backend coverage is broad; frontend has no tests/lint/a11y/E2E.' }, evidence: { ar: '1,799 اختبار Backend يُجمع؛ frontend scripts build/type-check.', en: '1,799 Backend tests collect; frontend scripts only build/type-check.' }, nextGate: { ar: 'Minimal quality harness واختبارات الرحلات المالية والعامة.', en: 'Minimal quality harness plus financial/public journey tests.' }, scope: 'all', phase: 'ui-foundation' },
...
export const SMART_EXECUTION_PHASES: SmartExecutionPhase[] = [
  ...
  { id: 'containment', order: 1, status: 'ready', title: { ar: 'احتواء الأخطار الحرجة', en: 'Critical-risk containment' }, purpose: { ar: 'تحقق من exposure؛ Public containment أولًا إن كان مكشوفًا، وإلا عقد الذرّية المالية أولًا.', en: 'Check exposure; public containment first if exposed, otherwise financial atomicity first.' }, dependencies: { ar: 'اعتماد مراجعة Gate 0.', en: 'Approval of Gate 0 review.' }, exitGate: { ar: 'لا self-order غير مقصود ولا أثر مالي يضيع بصمت في المسار المختار.', en: 'No unintended self-ordering or silently lost financial effect in the selected path.' } },
  { id: 'admin-security', order: 2, status: 'locked', title: { ar: 'أمان مركز التحكم', en: 'Control-plane security' }, purpose: { ar: 'حماية Super Admin والجلسات وTOTP وstep-up والتدقيق.', en: 'Protect Super Admin, sessions, TOTP, step-up, and audit.' }, dependencies: { ar: 'Gate 1.', en: 'Gate 1.' }, exitGate: { ar: 'منع escalation/self-lockout بالتزامن في السيرفر.', en: 'Server-side escalation/self-lockout prevention under concurrency.' } },
  ...
  { id: 'public-reference', order: 6, status: 'ready', title: { ar: 'Public Phase 0: تثبيت المرجع', en: 'Public Phase 0: freeze the reference' }, purpose: { ar: 'خريطة routes وصور Desktop/Mobile وأصول ومحتوى وAPI وKeep/Adapt/Remove فقط.', en: 'Route map, desktop/mobile screenshots, assets, content, APIs, and Keep/Adapt/Remove only.' }, dependencies: { ar: 'Gate 0 وموافقة Mohamed؛ لا يحتاج تغيير منتج.', en: 'Gate 0 and Mohamed’s approval; no product change is required.' }, exitGate: { ar: 'تقرير قابل للمراجعة بلا نقل كود أو Backend أو بيانات قديمة.', en: 'A reviewable report with no code, backend, or legacy-data migration.' } },
