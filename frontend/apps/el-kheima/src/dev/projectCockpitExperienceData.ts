...
export const CRITICAL_FINDINGS: CriticalFinding[] = [
  ...
  { id: 'superadmin', icon: 'shield-warning', severity: 'high', title: { ar: 'حماية الهوية — دورة الجلسة اتأمّنت، step-up/TOTP باقيان', en: 'Identity safeguards — session lifecycle secured, step-up/TOTP remain' }, impact: { ar: 'Gate 2A أغلقت ثوابت السوبر أدمن، وGate 2B1 أصلحت تغيير/reset كلمة السر وإبطال الجلسات وrefresh race وتسريب سر TOTP لطرف ثالث. ما زال bootstrap الافتراضي وفرض TOTP وstep-up والتدقيق مفتوحين.', en: 'Gate 2A closed Super Admin invariants; Gate 2B1 fixed password change/reset, session revocation, the refresh race, and third-party TOTP-secret disclosure. Default bootstrap, TOTP enforcement, step-up, and audit remain open.' }, evidence: { ar: 'Gate 2B1: 1,903 اختبار ناجح + 1 PostgreSQL refresh concurrency حقيقي؛ تنتظر مراجعة Claude المستقلة. راجع docs/audits/gate-2b1-auth-session-lifecycle.md.', en: 'Gate 2B1: 1,903 passing tests plus one real PostgreSQL refresh-concurrency proof; pending independent Claude review. See docs/audits/gate-2b1-auth-session-lifecycle.md.' }, action: { ar: 'راجع واعتمد Gate 2B1 أولًا؛ بعدها صمّم bootstrap آمن وTOTP fail-closed، ثم recent-auth/step-up والتدقيق في دفعة مستقلة.', en: 'Review and accept Gate 2B1 first; then design safe bootstrap and fail-closed TOTP, followed by recent-auth/step-up and audit in a separate slice.' }, source: 'docs/audits/gate-2b1-auth-session-lifecycle.md', scope: 'core', phase: 'superadmin-backend' },
...
export const READINESS_LENSES: ReadinessLens[] = [
  ...
  { id: 'authorization', icon: 'key', title: { ar: 'الصلاحيات والعزل', en: 'Authorization and isolation' }, state: 'risk', severity: 'high', finding: { ar: 'Gate 2A أثبتت ثوابت Super Admin، وGate 2B1 أمّنت دورة الجلسة لكنها تنتظر مراجعة مستقلة. عزل النطاق وstep-up ما زالا غير مكتملين.', en: 'Gate 2A proved the Super Admin invariants, and Gate 2B1 secured the session lifecycle but still awaits independent review. Scope isolation and step-up remain incomplete.' }, evidence: { ar: 'حماية last-admin وrefresh rotation مُختبرتان تحت تزامن PostgreSQL حقيقي؛ التفاصيل في تقريري Gate 2A و2B1.', en: 'The last-admin guard and refresh rotation are tested under real PostgreSQL concurrency; see the Gate 2A and 2B1 reports.' }, nextGate: { ar: 'اعتماد Gate 2B1، ثم bootstrap/TOTP وrecent-auth/step-up، وبعدها عزل الفرع/المنفذ.', en: 'Accept Gate 2B1, then address bootstrap/TOTP and recent-auth/step-up, followed by branch/outlet isolation.' }, scope: 'core', phase: 'superadmin-backend' },
  ...
  { id: 'testing', icon: 'clipboard', title: { ar: 'Testing والجودة', en: 'Testing and quality' }, state: 'risk', severity: 'high', finding: { ar: 'Backend واسع؛ Frontend بلا tests/lint/a11y/E2E.', en: 'Backend coverage is broad; frontend has no tests/lint/a11y/E2E.' }, evidence: { ar: '1,875 اختبار Backend يُجمع؛ frontend scripts build/type-check فقط.', en: '1,875 Backend tests collect; frontend scripts only build and type-check.' }, nextGate: { ar: 'Minimal quality harness واختبارات الرحلات المالية والعامة.', en: 'Minimal quality harness plus financial/public journey tests.' }, scope: 'all', phase: 'ui-foundation' },
...
export const SMART_EXECUTION_PHASES: SmartExecutionPhase[] = [
  ...
  { id: 'admin-security', order: 2, status: 'ready', title: { ar: 'أمان مركز التحكم — Gate 2B1 تنتظر المراجعة', en: 'Control-plane security — Gate 2B1 pending review' }, purpose: { ar: 'حماية Super Admin والجلسات وTOTP وstep-up والتدقيق.', en: 'Protect Super Admin, sessions, TOTP, step-up, and audit.' }, dependencies: { ar: 'Gate 2A مُعتمَدة؛ Gate 2B1 أمّنت كلمة السر والجلسات وQR الخاص بـTOTP وتنتظر مراجعة Claude. Gate 2B2/2B3 لم تبدأا.', en: 'Gate 2A is accepted; Gate 2B1 secured passwords, sessions, and TOTP QR confidentiality and awaits Claude review. Gates 2B2/2B3 have not started.' }, exitGate: { ar: 'اعتماد Gate 2B1 أولًا، ثم bootstrap آمن وفرض TOTP وstep-up/audit بأدلة مستقلة.', en: 'Accept Gate 2B1 first, then prove safe bootstrap, TOTP enforcement, and step-up/audit in separate slices.' } },
