...
export const CRITICAL_FINDINGS: CriticalFinding[] = [
  ...
  { id: 'public-trust', icon: 'qrcode', severity: 'high', title: { ar: 'QR الآمن يحتاج قبولًا ميدانيًا', en: 'Secure QR needs field acceptance' }, impact: { ar: 'Gate 8 أغلقت حدود الثقة البرمجية؛ الخطأ المتبقي المحتمل في الكاميرا أو الطباعة أو عنوان الموقع لا يظهر بالاختبارات الآلية.', en: 'Gate 8 closed the code trust boundary; camera, print, or public-site URL issues can still escape automated tests.' }, evidence: { ar: 'token/session/dedupe والطلب والدفع اجتازوا الاختبارات وتزامن PostgreSQL؛ لا تجربة هاتف أو QR مطبوع بعد.', en: 'Token/session/dedupe plus order/payment passed tests and PostgreSQL races; no phone or printed-QR trial yet.' }, action: { ar: 'نفّذ تجربة هاتف وطباعة على staging قبل النشر الميداني.', en: 'Run a phone and print trial on staging before field deployment.' }, source: 'docs/audits/gate-8-execution-brief.md', scope: 'dining', phase: 'qr-guest-service' },
  { id: 'superadmin', icon: 'shield-warning', severity: 'high', title: { ar: 'حماية الهوية — Gate 2 مُعتمَدة', en: 'Identity safeguards — Gate 2 accepted' }, impact: { ar: 'الحماية الأساسية مكتملة: bootstrap آمن، TOTP fail-closed، recovery أحادي الاستخدام، step-up مربوط بالغرض والجلسة، تدقيق موحّد، وعائلات refresh تكشف replay.', en: 'Core safeguards are complete: secure bootstrap, fail-closed TOTP, single-use recovery, purpose/session-bound step-up, unified audit, and replay-detecting refresh families.' }, evidence: { ar: 'Gate 2A→2B3B مُعتمَدة باختبارات suite كاملة وتزامن PostgreSQL ودورات migration.', en: 'Gates 2A→2B3B are accepted with full-suite, PostgreSQL-concurrency, and migration-cycle evidence.' }, action: { ar: 'لا توسّع control plane الآن؛ طبّق step-up على الأفعال المالية الحساسة عند تنفيذ Gate 4.', en: 'Do not expand the control plane now; apply step-up to sensitive financial actions in Gate 4.' }, source: 'docs/audits/gate-2b3b-auth-audit-session-defense.md', scope: 'core', phase: 'superadmin-backend' },
  { id: 'frontend-harness', icon: 'desktop', severity: 'high', title: { ar: 'بوابة اختبار Frontend (Gate 3C)', en: 'Frontend test quality gate (Gate 3C)' }, impact: { ar: 'اتبنت واتعمدت: validate:i18n + Vitest/VTU/jsdom/axe (60 اختبار) تغطي عزل لغات التطبيقين، المستخدم/الترمنال، الاتجاه والتوقيت والمكونات وa11y وsmoke. jsdom فقط — لا E2E متصفح.', en: 'Added and accepted: validate:i18n + Vitest/VTU/jsdom/axe (60 tests) covering app-locale isolation, user/terminal reconciliation, direction, resort time, shared components, a11y, and smoke. jsdom only — no browser E2E.' }, evidence: { ar: 'scripts: validate:i18n/test:unit/test:a11y/test:frontend + فحص production bundles.', en: 'Scripts: validate:i18n/test:unit/test:a11y/test:frontend plus production-bundle isolation check.' }, action: { ar: 'توسيع التغطية مع كل دفعة هجرة شاشات.', en: 'Grow coverage with each screen-migration batch.' }, source: 'frontend/apps/el-kheima/package.json', scope: 'core', phase: 'ui-foundation' },
]
...
export const READINESS_LENSES: ReadinessLens[] = [
  ...
  { id: 'public-security', icon: 'qrcode', title: { ar: 'أمان Public وQR', en: 'Public and QR security' }, state: 'verified', severity: 'foundation', finding: { ar: 'Gate 8 أضافت Service Location token وجلسة ضيف مجزأة وطلبات آمنة بلا IDs عامة موثوقة.', en: 'Gate 8 added Service Location tokens, hashed guest sessions, and safe requests without trusting public IDs.' }, evidence: { ar: 'تدوير وإلغاء، rate limits، dedupe/idempotency في قاعدة البيانات، عزل الفرع والجلسة، وAuditLog.', en: 'Rotation/revocation, rate limits, database dedupe/idempotency, branch/session isolation, and AuditLog.' }, nextGate: { ar: 'قبول كاميرا/طباعة على staging ثم Gate 9 للنشر.', en: 'Camera/print acceptance on staging, then Gate 9 deployment evidence.' }, scope: 'dining', phase: 'qr-guest-service' },
  ...
  { id: 'frontend', icon: 'desktop', title: { ar: 'Frontend architecture', en: 'Frontend architecture' }, state: 'verified', severity: 'foundation', finding: { ar: 'Gate 3 عزلت runtimes التطبيقين وأضافت package subpaths واختبارات، مع بقاء typed API debt يُراجع حسب الرحلة.', en: 'Gate 3 isolated both app runtimes and added package subpaths and tests; typed API debt remains for journey-level review.' }, evidence: { ar: 'type-check/build للتطبيقين + 60 Vitest + فحص production bundles.', en: 'Both app type-check/builds, 60 Vitest tests, and production-bundle inspection.' }, nextGate: { ar: 'Typed API/state review حسب الرحلة لا mass rewrite.', en: 'Typed API/state review per workflow, not a mass rewrite.' }, scope: 'core', phase: 'ui-foundation' },
  { id: 'design', icon: 'sparkles', title: { ar: 'Design System', en: 'Design system' }, state: 'verified', severity: 'foundation', finding: { ar: 'Tokens و43 مكوّنًا موجودة؛ Gate 3 اعتمدت baseline على الشيل وشاشات مرجعية، لكن adoption الكامل ما زال Gate 5.', en: 'Tokens and 43 components exist; Gate 3 accepted a shell/reference-screen baseline, while full adoption remains Gate 5.' }, evidence: { ar: '@resort-os/ui مع AppModal keyboard/focus واختبارات axe.', en: '@resort-os/ui with keyboard/focus-safe AppModal and axe tests.' }, nextGate: { ar: 'Migration على batches مع توسيع ratchet.', en: 'Batched migration with an expanding ratchet.' }, scope: 'core', phase: 'ui-foundation' },
  ...
  { id: 'a11y', icon: 'verified', title: { ar: 'Accessibility', en: 'Accessibility' }, state: 'verified', severity: 'foundation', finding: { ar: 'Gate 3 أضافت axe/component gate وقوّت AppModal للـkeyboard/focus؛ browser وscreen-reader الحقيقيان ما زالا مطلوبين لكل رحلة.', en: 'Gate 3 added an axe/component gate and hardened AppModal keyboard/focus behavior; real browser and screen-reader evidence is still needed per journey.' }, evidence: { ar: 'اختبارات AppInput/AppButton/AppModal + Profile ar/en بلا critical/serious axe violations.', en: 'AppInput/AppButton/AppModal tests plus Profile ar/en with no critical/serious axe violations.' }, nextGate: { ar: 'WCAG 2.2 AA عملي على الرحلات الحرجة والأجهزة المستهدفة.', en: 'Practical WCAG 2.2 AA checks on critical journeys and target devices.' }, scope: 'core', phase: 'ui-foundation' },
  { id: 'i18n', icon: 'language', title: { ar: 'اللغة وRTL/LTR', en: 'Localization and RTL/LTR' }, state: 'verified', severity: 'foundation', finding: { ar: 'Gate 3 مُعتمَدة: مصدر Staff مستقل (ar/en)، Public مستقل (ar/en/ru/it)، اتجاه مركزي، والشيل + Profile/Sessions/Settings مهاجرة. لسه ~40 شاشة بنص عربي ثابت.', en: 'Gate 3 is accepted: isolated Staff (ar/en) and Public (ar/en/ru/it) runtimes, central direction, and migrated shell + Profile/Sessions/Settings. ~40 screens still hold hard-coded Arabic.' }, evidence: { ar: 'preferred_language محفوظ؛ 3115 مفتاح؛ 60 اختبار؛ فحص الحزم يمنع تداخل runtime.', en: 'Saved preferred_language, 3115 keys, 60 tests, and bundle inspection preventing runtime cross-contamination.' }, nextGate: { ar: 'هجرة باقي الشاشات بدفعات مع توسيع القائمة الصارمة.', en: 'Batched migration of remaining screens, extending the strict list.' }, scope: 'core', phase: 'staff-i18n-foundation' },
  { id: 'testing', icon: 'clipboard', title: { ar: 'Testing والجودة', en: 'Testing and quality' }, state: 'verified', severity: 'foundation', finding: { ar: 'Backend واسع، وGate 3 أضافت بوابة Frontend صغيرة قابلة للصيانة؛ E2E متصفح حقيقي ما زال غير موجود.', en: 'Backend coverage is broad, and Gate 3 added a small maintainable frontend gate; real-browser E2E remains absent.' }, evidence: { ar: '1992 backend ناجح + 20 skipped؛ 60 frontend؛ validate:i18n وtype-check/build للتطبيقين.', en: '1992 backend passed + 20 skipped; 60 frontend; validate:i18n and both app type-check/builds.' }, nextGate: { ar: 'وسّع الاختبارات حسب الرحلة، وأضف browser E2E فقط عند اختيار رحلة حرجة مناسبة.', en: 'Expand tests per journey and add browser E2E only when a suitable critical flow is selected.' }, scope: 'all', phase: 'ui-foundation' },
  ...
  { id: 'realtime', icon: 'online', title: { ar: 'Realtime وOffline', en: 'Realtime and offline' }, state: 'verified', severity: 'foundation', finding: { ar: 'طابور الطلبات يستخدم WebSocket مع polling احتياطي، والضيف يستعيد الحالة والجلسة بعد إعادة التحميل.', en: 'The request queue uses WebSocket with polling fallback, while guests recover session and status after reload.' }, evidence: { ar: 'اختبارات retry/dedupe/expiry واستعادة الحالة + polling للموظفين كل 20 ثانية.', en: 'Retry/dedupe/expiry and recovery tests plus 20-second staff polling.' }, nextGate: { ar: 'اختبار انقطاع شبكة حقيقي على هاتف قبل الاستخدام الميداني.', en: 'Run a real phone network-interruption trial before field use.' }, scope: 'dining', phase: 'qr-guest-service' },
...
export const SMART_EXECUTION_PHASES: SmartExecutionPhase[] = [
  ...
  { id: 'admin-security', order: 2, status: 'complete', title: { ar: 'أمان مركز التحكم — Gate 2 مكتملة', en: 'Control-plane security — Gate 2 complete' }, purpose: { ar: 'حماية Super Admin والجلسات وTOTP وstep-up والتدقيق.', en: 'Protect Super Admin, sessions, TOTP, step-up, and audit.' }, dependencies: { ar: 'Gate 2A حتى Gate 2B3B مُعتمَدة كلها.', en: 'Gates 2A through 2B3B are all accepted.' }, exitGate: { ar: 'حماية آخر Super Admin، bootstrap/TOTP/recovery، step-up، audit، وعائلات الجلسات مثبتة باختبارات وتزامن حقيقي.', en: 'Last-Super-Admin safeguards, bootstrap/TOTP/recovery, step-up, audit, and session families are proven with tests and real concurrency.' } },
  { id: 'ui-foundation', order: 3, status: 'complete', title: { ar: 'أساس اللغة والديزاين والاختبار', en: 'Language, design, and test foundation' }, purpose: { ar: 'i18n/RTL/LTR، adoption للمكونات، وquality harness صغير.', en: 'i18n/RTL/LTR, component adoption, and a minimal quality harness.' }, dependencies: { ar: 'Gate 2 للشاشات الحساسة.', en: 'Gate 2 for sensitive screens.' }, exitGate: { ar: 'Gate 3 مُعتمَدة: shell + Profile/Sessions/Settings مرجعية، عزل Staff/Public، و60 اختبار frontend؛ باقي الشاشات Gate 5.', en: 'Gate 3 accepted: reference shell + Profile/Sessions/Settings, Staff/Public isolation, and 60 frontend tests; remaining views belong to Gate 5.' } },
  { id: 'dining', order: 4, status: 'ready', title: { ar: 'Dining المالي والتشغيلي', en: 'Dining finance and operations' }, purpose: { ar: 'Payment/shift/method/idempotency وownership والتزامن.', en: 'Payment/shift/method/idempotency, ownership, and concurrency.' }, dependencies: { ar: 'Gate 1 المالي + سياسات Gate 2 — مكتملتان.', en: 'Financial Gate 1 + Gate 2 policies — both satisfied.' }, exitGate: { ar: 'كل بيع يظهر مرة واحدة في الوردية والفوليو والدفتر، مع failure/concurrency proofs.', en: 'Every sale appears exactly once in shift, folio, and ledger, backed by failure/concurrency proofs.' } },
  ...
  { id: 'public-migration', order: 7, status: 'complete', title: { ar: 'نقل الموقع العام على دفعات', en: 'Batched public-site migration' }, purpose: { ar: 'نقل التصميم والصفحات المختارة إلى apps/public مع Backend الحالي.', en: 'Move selected design and pages into apps/public against the current backend.' }, dependencies: { ar: 'الدفعات الخمس العامة + رحلة QR في Gate 8 مكتملة.', en: 'Five public batches plus the Gate 8 QR journey are complete.' }, exitGate: { ar: 'Visual/API comparison وبناء واختبار لكل batch.', en: 'Visual/API comparison, build, and tests per batch.' } },
  { id: 'qr', order: 8, status: 'complete', title: { ar: 'QR وGuest Service', en: 'QR and guest service' }, purpose: { ar: 'Service Location/token/session/view_and_call/realtime/dedupe وربط الويتر والكاشير.', en: 'Service Location/token/session/view_and_call/realtime/dedupe plus waiter/cashier integration.' }, dependencies: { ar: 'اكتملت برمجيًا؛ القبول الميداني ينتقل إلى staging/Gate 9.', en: 'Code-complete; field acceptance moves to staging/Gate 9.' }, exitGate: { ar: 'الرحلة الآلية من الجلسة إلى الطلب والدفع اجتازت؛ الكاميرا والطباعة والشبكة الحقيقية باقية.', en: 'The automated session-to-request/payment journey passes; real camera, print, and network trials remain.' } },
