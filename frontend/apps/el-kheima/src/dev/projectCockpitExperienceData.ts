...
export const CRITICAL_FINDINGS: CriticalFinding[] = [
  ...
  { id: 'superadmin', icon: 'shield-warning', severity: 'high', title: { ar: 'حماية الهوية — Gate 2B1 مُعتمَدة، step-up/TOTP باقيان', en: 'Identity safeguards — Gate 2B1 accepted, step-up/TOTP remain' }, impact: { ar: 'Gate 2A أغلقت ثوابت السوبر أدمن، وGate 2B1 أصلحت تغيير/reset كلمة السر وإبطال الجلسات وrefresh race وتسريب سر TOTP لطرف ثالث. ما زال bootstrap الافتراضي وفرض TOTP وstep-up والتدقيق مفتوحين.', en: 'Gate 2A closed Super Admin invariants; Gate 2B1 fixed password change/reset, session revocation, the refresh race, and third-party TOTP-secret disclosure. Default bootstrap, TOTP enforcement, step-up, and audit remain open.' }, evidence: { ar: 'Gate 2B1 مُعتمَدة بعد مراجعة Claude: 1,903 اختبار ناجح + 1 PostgreSQL refresh concurrency حقيقي. راجع docs/audits/gate-2b1-auth-session-lifecycle.md.', en: 'Gate 2B1 is accepted after Claude review: 1,903 passing tests plus one real PostgreSQL refresh-concurrency proof. See docs/audits/gate-2b1-auth-session-lifecycle.md.' }, action: { ar: 'Gate 2B2: صمّم bootstrap واستردادًا آمنين قبل جعل TOTP fail-closed؛ ثم recent-auth/step-up والتدقيق في دفعة مستقلة.', en: 'Gate 2B2: design safe bootstrap and recovery before making TOTP fail closed; then address recent-auth/step-up and audit separately.' }, source: 'docs/audits/gate-2b1-auth-session-lifecycle.md', scope: 'core', phase: 'superadmin-backend' },
...
export const READINESS_LENSES: ReadinessLens[] = [
  ...
  { id: 'authorization', icon: 'key', title: { ar: 'الصلاحيات والعزل', en: 'Authorization and isolation' }, state: 'risk', severity: 'high', finding: { ar: 'Gate 2A وGate 2B1 أثبتتا ثوابت Super Admin ودورة الجلسة. عزل النطاق وstep-up ما زالا غير مكتملين.', en: 'Gates 2A and 2B1 proved the Super Admin invariants and session lifecycle. Scope isolation and step-up remain incomplete.' }, evidence: { ar: 'حماية last-admin وrefresh rotation مُختبرتان تحت تزامن PostgreSQL حقيقي؛ التفاصيل في تقريري Gate 2A و2B1.', en: 'The last-admin guard and refresh rotation are tested under real PostgreSQL concurrency; see the Gate 2A and 2B1 reports.' }, nextGate: { ar: 'bootstrap/TOTP آمنان ثم recent-auth/step-up، وبعدها عزل الفرع/المنفذ.', en: 'Safe bootstrap/TOTP, then recent-auth/step-up, followed by branch/outlet isolation.' }, scope: 'core', phase: 'superadmin-backend' },
...
export const SMART_EXECUTION_PHASES: SmartExecutionPhase[] = [
  ...
  { id: 'admin-security', order: 2, status: 'ready', title: { ar: 'أمان مركز التحكم — Gate 2B1 مُعتمَدة', en: 'Control-plane security — Gate 2B1 accepted' }, purpose: { ar: 'حماية Super Admin والجلسات وTOTP وstep-up والتدقيق.', en: 'Protect Super Admin, sessions, TOTP, step-up, and audit.' }, dependencies: { ar: 'Gate 2A وGate 2B1 مُعتمَدتان؛ Gate 2B2 تبدأ بتصميم bootstrap/TOTP والاسترداد، وGate 2B3 للـstep-up/audit.', en: 'Gates 2A and 2B1 are accepted; Gate 2B2 begins with bootstrap/TOTP and recovery design, while Gate 2B3 covers step-up/audit.' }, exitGate: { ar: 'bootstrap بلا سر افتراضي، enrollment لا يمكن الاستحواذ عليه، recovery يمنع lockout، ثم فرض TOTP بدليل حي.', en: 'Bootstrap without a default secret, enrollment resistant to takeover, recovery preventing lockout, then live proof of TOTP enforcement.' } },
