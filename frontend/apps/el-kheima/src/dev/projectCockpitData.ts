...
export const PROJECT_MODULES: ProjectModule[] = [
  {
    ...
    health: 'attention',
    note: {
      ar: 'Gate 2 وGate 3 مُعتمَدتان. تفضيل ar/en محفوظ في الحساب، Runtime الموظفين معزول عن Public، الاتجاه والتوقيت مركزيان، وتبنّي Design System مرجعي مع validate:i18n + Vitest/axe (60 اختبار). لسه ~40 شاشة غير مهاجرة — مش ثنائي اللغة بالكامل بعد.',
      en: 'Gates 2 and 3 are accepted. The account-saved ar/en preference, Staff/Public runtime isolation, central direction/resort time, reference design-system adoption, and validate:i18n + Vitest/axe harness (60 tests) are in place. ~40 screens remain unmigrated — not yet fully bilingual.',
    },
  },
  {
    id: 'dining',
...
export const PROJECT_ROADMAP: RoadmapStep[] = [
  ...
  },
  {
    ...
    outcome: { ar: 'Gate 2A/2B1/2B2/2B3A/2B3B مُعتمَدة كلها. الجلسات مرتبطة بـsid وتُلغى فورًا، والتدقيق وreplay وstep-up محميون ومختبرون. لا push.', en: 'Gates 2A/2B1/2B2/2B3A/2B3B are all accepted. Sessions are sid-bound and revoke immediately; audit, replay, and step-up controls are tested. Nothing pushed.' },
    gate: { ar: '1,975 اختبارًا + 4/4 refresh-family + 3/3 step-up + 2/2 super-admin على PostgreSQL، migration cycle وبناء frontend ناجحان.', en: '1,975 tests plus 4/4 refresh-family, 3/3 step-up, and 2/2 super-admin live-PostgreSQL proofs; migration cycle and frontend build passed.' },
  },
  {
...
export const PROJECT_RISKS: ProjectRisk[] = [
  ...
  },
  {
    ...
    severity: 'high',
    area: 'core',
    title: { ar: 'أمان الهوية — Gate 2 مُعتمَدة', en: 'Identity security — Gate 2 accepted' },
    detail: { ar: 'Gate 2B3B اجتازت مراجعة Codex النهائية بعد ربط access token بالجلسة وإغلاق فجوات الملكية والتزامن والتدقيق. 1,975 اختبار + 9 اختبارات تزامن حقيقية مرتبطة بالهوية على PostgreSQL. لا push.', en: 'Gate 2B3B passed final Codex review after session-binding access tokens and closing ownership, concurrency, and rejection-audit gaps. 1,975 tests plus 9 identity-related live-PostgreSQL concurrency proofs. Nothing pushed.' },
  },
  {
...
export const PROMPT_SUGGESTIONS: PromptSuggestion[] = [
  ...
  {
    id: 'gate4-dining-integrity',
    title: { ar: 'نفّذ Gate 4 — سلامة Dining المالية والتشغيلية', en: 'Implement Gate 4 — Dining financial and operational integrity' },
    description: { ar: 'Gate 3 مُعتمَدة. التالي حزمة Dining كبيرة لكن مضبوطة: الدفع والوردية وطرق الدفع وidempotency والتسوية والـvoid/refund/discount والملكية ومنع الطلب النشط المكرر، مع failure/concurrency proofs.', en: 'Gate 3 is accepted. Next is a large but controlled Dining package: payments, shifts, payment methods, idempotency, reconciliation, void/refund/discount controls, ownership, and duplicate-active-order prevention, backed by failure/concurrency proofs.' },
    mode: 'implement',
    scope: 'dining',
    phase: 'financial-atomicity',
    objective: {
      ar: 'اقرأ docs/audits/gate-4-execution-brief.md كاملًا مع تقارير Gate 1B وGate 2 وخارطة التنفيذ. افحص الكود والاختبارات الحالية أولًا، ثم نفّذ شرائح Gate 4 المحددة فقط بمعاملات صحيحة وidempotency وaudit واختبارات failure/concurrency حقيقية على PostgreSQL. لا تغيّر Public/QR أو تعمل mass rewrite، ولا commit أو push؛ توقف لمراجعة Codex في النهاية.',
      en: 'Read docs/audits/gate-4-execution-brief.md in full with the Gate 1B and Gate 2 reports and the roadmap. Inspect existing code and tests first, then implement only the defined Gate 4 slices with sound transactions, idempotency, audit, and real PostgreSQL failure/concurrency proofs. Do not change Public/QR or mass-rewrite; do not commit or push, then stop for Codex review.',
    },
    decisionIds: ['brand', 'dining-unified', 'audit-before-phases', 'staged-review', 'finance-first'],
  },
  {
    id: 'i18n-foundation',
