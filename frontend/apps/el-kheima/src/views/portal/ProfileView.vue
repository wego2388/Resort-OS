...
// /hr/me/profile always 404s for an account with no linked HR Employee
// record (e.g. the super_admin bootstrap account) — that used to render as
// a scary full-page error (and, worse, hid the password/PIN sections below
// it too, which have nothing to do with the HR record). Skip the call
// entirely when auth.employeeId is null and show a neutral notice instead
...
<template>
  <div class="space-y-4">
    ...
    <!-- HR employee profile card — only for accounts with a linked Employee record -->
    <div v-if="auth.employeeId == null" class="text-center py-8 text-gray-500 dark:text-gray-400 bg-white dark:bg-surface rounded-2xl border border-stone-200 dark:border-border">
      <div class="text-3xl mb-2" aria-hidden="true">ℹ️</div>
      <p class="font-medium">{{ t('backoffice.profile.noEmployeeRecord') }}</p>
    </div>

    <div v-else-if="loading" class="flex flex-col items-center justify-center py-12 text-gray-400 dark:text-gray-400 gap-3">
      <AppSpinner size="lg" />
    ...
    <div v-else-if="profile" class="bg-white dark:bg-surface rounded-2xl border border-stone-200 dark:border-border p-6 shadow-sm">
    ...
    <!-- Change password + operational PIN — available to every signed-in
         account regardless of HR employee linkage (was previously nested
         inside the employee-profile-only block above, so an account with
         no HR record — e.g. the super_admin bootstrap account — had no way
         to reach either from this screen at all). -->
